    ...
    internal sealed class EpmTargetTree
    {
        ...
        internal void Add(EntityPropertyMappingInfo epmInfo)
        {
            ...
            if (epmInfo.MultiValueStatus == EntityPropertyMappingMultiValueStatus.MultiValueItemProperty)
            {
                ...
                // and all the ones which have EpmSyndicationParent.Entry (Title, Updated etc.) are not valid targets for multiValues.
            ...
            if (currentSegment.EpmInfo != null)
            {
                ...
                else
                {
                    Debug.Assert(
                        epmInfo.MultiValueStatus != EntityPropertyMappingMultiValueStatus.MultiValueProperty,
                        "Only non-multiValue properties should get here, we cover the rest above.");

                    // Two EpmAttributes with same TargetName in the inheritance hierarchy
                    throw new ODataException(Strings.EpmTargetTree_DuplicateEpmAttributesWithSameTargetName(EpmTargetTree.GetPropertyNameFromEpmInfo(currentSegment.EpmInfo), currentSegment.EpmInfo.DefiningType.ODataFullName(), currentSegment.EpmInfo.Attribute.SourcePath, epmInfo.Attribute.SourcePath));
                }
            }
                    ...
                    case AtomConstants.AtomCategoryElementName:
                        if (!segment.SubSegments.Any(s => s.EpmInfo.Attribute.TargetSyndicationItem == SyndicationItemProperty.CategoryTerm))
                        {
                            throw new ODataException(Strings.EpmTargetTree_ConditionalMappingCategoryTermIsRequired(
                                ...
                                segment.SubSegments[0].EpmInfo.DefiningType.ODataFullName(),
