...
namespace Microsoft.Data.OData.Json
{
    ...
    internal sealed class ODataJsonCollectionReader : ODataCollectionReaderCore
    {
        ...
        internal ODataJsonCollectionReader(ODataJsonInputContext jsonInputContext, IEdmTypeReference expectedItemTypeReference)
            : base(jsonInputContext, expectedItemTypeReference)
        {
        ...
        protected override bool ReadAtStartImplementation()
        {
            ...
            Debug.Assert(this.jsonCollectionDeserializer.JsonReader.NodeType == JsonNodeType.None, "Pre-Condition: expected JsonNodeType.None");

            // read the data wrapper depending on whether we are reading a request or response
            this.jsonCollectionDeserializer.ReadDataWrapperStart();

            if (this.IsResultsWrapperExpected && this.jsonCollectionDeserializer.JsonReader.NodeType != JsonNodeType.StartObject)
            {
                throw new ODataException(Strings.ODataJsonCollectionReader_CannotReadWrappedCollectionStart(this.jsonCollectionDeserializer.JsonReader.NodeType));
            }

            if (!this.IsResultsWrapperExpected && this.jsonCollectionDeserializer.JsonReader.NodeType != JsonNodeType.StartArray)
            {
                throw new ODataException(Strings.ODataJsonCollectionReader_CannotReadCollectionStart(this.jsonCollectionDeserializer.JsonReader.NodeType));
            }

            // read the start of the collection until we find the content array
            ODataCollectionResult collection = this.jsonCollectionDeserializer.ReadCollectionStart(this.IsResultsWrapperExpected);

            this.jsonCollectionDeserializer.JsonReader.ReadStartArray();

            this.EnterScope(ODataCollectionReaderState.CollectionStart, collection);
        ...
        protected override bool ReadAtCollectionStartImplementation()
        {
            ...
            {
                object item = this.jsonCollectionDeserializer.ReadCollectionItem(this.ExpectedItemTypeReference);
        ...
        /// Post-Condition: JsonNodeType.EndOfInput     nothing else to read
        /// </remarks>
        protected override bool ReadAtCollectionEndImplementation()
        {
            ...
            this.jsonCollectionDeserializer.ReadDataWrapperEnd();
            Debug.Assert(this.jsonInputContext.JsonReader.NodeType == JsonNodeType.EndOfInput, "Expected JSON reader to have reached the end of input.");
