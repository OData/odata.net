...
namespace Microsoft.Data.OData.Atom
{
    ...
    internal sealed class ODataAtomCollectionDeserializer : ODataAtomPropertyAndValueDeserializer
    {
        ...
        private readonly string ODataCollectionItemElementName;
        #endregion
        ...
        /// <returns>An <see cref="ODataCollectionResult"/> representing the collection-level information. Currently this only contains 
        ...
        /// </remarks>
        internal ODataCollectionResult ReadCollectionStart(out bool isCollectionElementEmpty)
        {
            DebugUtils.CheckNoExternalCallers();
            ...
            {
                throw new ODataException(Strings.ODataAtomCollectionDeserializer_TopLevelCollectionElementWrongNamespace(this.XmlReader.NamespaceURI));
            }
            ...
            ODataCollectionResult collection = new ODataCollectionResult();
            ...
            collection.Name = this.XmlReader.LocalName;
        ...
        /// </remarks>
        internal object ReadCollectionItem(IEdmTypeReference expectedItemType)
        {
            DebugUtils.CheckNoExternalCallers();
            this.XmlReader.AssertNotBuffering();
            this.AssertXmlCondition(XmlNodeType.Element);

            // the caller should gaurantee that we are reading elements in the OData namespace.
            Debug.Assert(this.XmlReader.NamespaceEquals(this.XmlReader.ODataNamespace), "The 'element' node should be in the OData Namespace");

            // make sure that the item is named as 'element'.
            if (!this.XmlReader.LocalNameEquals(this.ODataCollectionItemElementName))
            {
                throw new ODataException(Strings.ODataAtomCollectionDeserializer_WrongCollectionItemElementName(this.XmlReader.LocalName));
            }

            object item = this.ReadNonEntityValue(expectedItemType, this.duplicatePropertyNamesChecker);
