...
namespace Microsoft.Data.OData.Atom
{
    ...
    internal sealed class EpmCustomWriter : EpmWriter
    {
            ...
            ODataWriterBehavior writerBehavior)
        {
            ...
            EpmCustomWriter epmWriter = new EpmCustomWriter(model, version, writerBehavior);
            ...
            ref string alreadyDeclaredPrefix)
        {
            ...
            if (targetSegment.HasContent)
            {
                ...
                {
                    // this must be element target otherwise GetEntryPropertyValueAsText() above would throw.
                    Debug.Assert(this.Version >= ODataVersion.V3 && !targetSegment.IsAttribute, "version >= ODataVersion.V3  && !targetSegment.IsAttribute");
            ...
            IEdmEntityTypeReference entityType)
        {
            ...
            // We can safely ignore both nullOnParentProperty and valueTypeReference since those are only usefull for multivalues
            // and we don't support custom mappings for multivalues.
            ...
            {
                // In V3 when we use new format for null values using the m:null attribute.
                // If we're mapping into an attribute we should fail since we can't write null to attribute in V3.
                // If we're writing into element return true null, to use the m:null attribute.
                ...
                if (this.Version >= ODataVersion.V3)
                {
                    ...
                    // custom mapping to element with a null value, caller will write m:null.
                    return null;
                }
