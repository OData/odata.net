...
namespace Microsoft.Data.OData.Atom
{
    ...
    internal sealed class ODataAtomServiceDocumentDeserializer : ODataAtomDeserializer
    {
        ...
        private readonly string AtomPublishingCategoriesElementName = "categories";

        /// <summary>The name of the 'accept' element of a service document.</summary>
        private readonly string AtomPublishingAcceptElementName = "accept";
                ...
                do
                {
                    ...
                    switch (this.XmlReader.NodeType)
                    {
                        case XmlNodeType.Element:
                            ...
                            {
                                // skip all other elements including 'ATOM:title'
                                this.XmlReader.Skip();
                            }
        ...
        private ODataResourceCollectionInfo ReadCollectionElement()
        {
            ...
            string href = this.XmlReader.GetAttributeValue(this.AtomHRefAttributeName, this.EmptyNamespace);
            ...
            if (!this.XmlReader.IsEmptyElement)
            {
                ...
                do
                {
                    switch (this.XmlReader.NodeType)
                    {
                        case XmlNodeType.Element:
                            if (this.XmlReader.NamespaceEquals(this.AtomPublishingNamespace)
                                && !this.XmlReader.LocalNameEquals(this.AtomPublishingCategoriesElementName)
                                && !this.XmlReader.LocalNameEquals(this.AtomPublishingAcceptElementName))
                            {
                                // Throw error if we find anything other then a 'app:categories' or an 'app:accept' element in the ATOM publishing namespace.
                                throw new ODataException(Strings.ODataAtomServiceDocumentDeserializer_UnexpectedElementInCollection(this.XmlReader.LocalName));
                            }
