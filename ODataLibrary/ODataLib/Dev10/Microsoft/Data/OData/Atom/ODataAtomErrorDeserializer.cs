...
namespace Microsoft.Data.OData.Atom
{
    ...
    internal sealed class ODataAtomErrorDeserializer : ODataAtomDeserializer
    {
        ...
        /// Post-Condition: any                   - The node after the m:error end element (or the empty m:error start element)
        /// </remarks>
        internal static ODataError ReadErrorElement(BufferingXmlReader xmlReader)
        {
            DebugUtils.CheckNoExternalCallers();
                            ...
                            if (xmlReader.NamespaceEquals(xmlReader.ODataMetadataNamespace))
                            {
                                switch (xmlReader.LocalName)
                                {
                                    ...
                                    case AtomConstants.ODataErrorMessageElementName:
                                        ...
                                        error.MessageLanguage = xmlReader.GetAttributeValue(xmlReader.XmlLangAttributeName, xmlReader.XmlNamespace);
                                    ...
                                    case AtomConstants.ODataInnerErrorElementName:
                                        ...
                                        error.InnerError = ReadInnerErrorElement(xmlReader);
                                        continue;
        ...
        internal ODataError ReadTopLevelError()
        {
            ...
            try
            {
                ...
                ODataError error = ReadErrorElement(this.XmlReader);
                this.ReadPayloadEnd();
                return error;
            }
            finally
        ...
        /// </remarks>
        private static ODataInnerError ReadInnerErrorElement(BufferingXmlReader xmlReader)
        {
            Debug.Assert(xmlReader != null, "this.XmlReader != null");
            Debug.Assert(xmlReader.NodeType == XmlNodeType.Element, "xmlReader.NodeType == XmlNodeType.Element");
            Debug.Assert(
                            ...
                            if (xmlReader.NamespaceEquals(xmlReader.ODataMetadataNamespace))
                            {
                                switch (xmlReader.LocalName)
                                {
                                    ...
                                    case AtomConstants.ODataInnerErrorInnerErrorElementName:
                                        ...
                                        innerError.InnerError = ReadInnerErrorElement(xmlReader);
                                        continue;
