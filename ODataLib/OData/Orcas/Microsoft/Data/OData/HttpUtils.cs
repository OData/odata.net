    ...
    internal static class HttpUtils
    {
        ...
        private static string ReadQuotedParameterValue(string parameterName, string headerText, ref int textIndex)
        {
            ...
            while (textIndex < headerText.Length)
            {
                ...
                {
                    if (!valueIsQuoted && !IsHttpToken(currentChar))
                    {
                        // If the given character is special, we stop processing.
                        break;
                    }
                    else if (valueIsQuoted && !IsValidInQuotedHeaderValue(currentChar))
                    {
                        throw new ODataException(Strings.HttpUtils_InvalidCharacterInQuotedParameterValue(headerText, currentChar, textIndex));
                    }
                }
