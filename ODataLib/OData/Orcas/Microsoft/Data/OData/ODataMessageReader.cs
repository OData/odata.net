    ...
    public sealed class ODataMessageReader : IDisposable
    {
        ...
        private MediaTypeResolver MediaTypeResolver
        {
            get
            {
                ...
                {
                    // Allow plain "application/json" and treat it the same as if it had "odata=verbose" when the version is less than 3.
                    bool shouldPlainAppJsonImplyVerboseJson = this.version < ODataVersion.V3;

                    bool shouldAppXmlAndAppAtomXmlBeInterchangeable = 
                        this.version < ODataVersion.V3 && this.settings.ReaderBehavior.FormatBehaviorKind == ODataBehaviorKind.WcfDataServicesClient;
                    
                    this.mediaTypeResolver = new MediaTypeResolver(shouldPlainAppJsonImplyVerboseJson, shouldAppXmlAndAppAtomXmlBeInterchangeable);
                }
        ...
        public IEnumerable<ODataPayloadKindDetectionResult> DetectPayloadKind()
        {
            ...
            if (this.TryGetSinglePayloadKindResultFromContentType(out payloadKindsFromContentType, out contentType, out contentEncoding))
            {
        ...
        public Task<IEnumerable<ODataPayloadKindDetectionResult>> DetectPayloadKindAsync()
        {
            ...
            return Task.Factory.Iterate(this.GetPayloadKindDetectionTasks(contentType, contentEncoding, payloadKindsFromContentType, detectedPayloadKinds))
                .FollowAlwaysWith(
                    t =>
                    {
                        // We are done sniffing; stop buffering.
        ...
        private void ProcessContentType(params ODataPayloadKind[] payloadKinds)
        {
            ...
            MediaType mediaType;
            this.format = MediaTypeUtils.GetFormatFromContentType(contentTypeHeader, payloadKinds, this.MediaTypeResolver, out mediaType, out this.encoding, out this.readerPayloadKind, out this.batchBoundary);
        }
        ...
        /// <param name="contentEncoding">The encoding from the content type or the default encoding from <see cref="MediaType" />.</param>
        ...
        private bool TryGetSinglePayloadKindResultFromContentType(out IEnumerable<ODataPayloadKindDetectionResult> payloadKindResults, out MediaType contentType, out Encoding contentEncoding)
        {
            ...
            string contentTypeHeader = this.GetContentTypeHeader();
            IList<ODataPayloadKindDetectionResult> payloadKindsFromContentType = MediaTypeUtils.GetPayloadKindsForContentType(contentTypeHeader, this.MediaTypeResolver, out contentType, out contentEncoding);
