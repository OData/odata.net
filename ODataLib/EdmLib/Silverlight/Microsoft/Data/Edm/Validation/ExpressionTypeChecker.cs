...
namespace Microsoft.Data.Edm.Validation
{
    ...
    public static class ExpressionTypeChecker
    {
        ...
        public static bool TryAssertType(this IEdmExpression expression, IEdmTypeReference type, out IEnumerable<EdmError> discoveredErrors)
        {
            ...
            switch (expression.ExpressionKind)
            {
                ...
                case EdmExpressionKind.TimeConstant:
                    ...
                    {
                        return TestTypeMatch(primitiveValue.Type, type, expression.Location(), out discoveredErrors);
                    }
                ...
                case EdmExpressionKind.Path:
                    return TryAssertPathAsType((IEdmPathExpression)expression, type, out discoveredErrors);
                case EdmExpressionKind.FunctionApplication:
                    ...
                    if (applyExpression.AppliedFunction != null)
                    {
                        ...
                        if (function != null)
                        {
                            return TestTypeMatch(function.ReturnType, type, expression.Location(), out discoveredErrors);
                        }
                    }
                ...
                case EdmExpressionKind.If:
                    return TryAssertIfAsType((IEdmIfExpression)expression, type, out discoveredErrors);
                case EdmExpressionKind.IsType:
                    return TestTypeMatch(EdmCoreModel.Instance.GetBoolean(false), type, expression.Location(), out discoveredErrors);
                case EdmExpressionKind.Record:
                    IEdmRecordExpression recordExpression = (IEdmRecordExpression)expression;
                    if (recordExpression.DeclaredType != null)
                    {
                        return TestTypeMatch(recordExpression.DeclaredType, type, expression.Location(), out discoveredErrors);
                    }

                    return TryAssertRecordAsType(recordExpression, type, out discoveredErrors);
                case EdmExpressionKind.Collection:
                    ...
                    {
                        return TestTypeMatch(collectionExpression.DeclaredType, type, expression.Location(), out discoveredErrors);
                    }

                    return TryAssertCollectionAsType(collectionExpression, type, out discoveredErrors);
                case EdmExpressionKind.Labeled:
                    return TryAssertType(((IEdmLabeledExpression)expression).Expression, type, out discoveredErrors);
                case EdmExpressionKind.AssertType:
                    return TestTypeMatch(((IEdmAssertTypeExpression)expression).Type, type, expression.Location(), out discoveredErrors);
        ...
        internal static bool TryAssertIfAsType(this IEdmIfExpression expression, IEdmTypeReference type, out IEnumerable<EdmError> discoveredErrors)
        {
            ...
            bool success = TryAssertType(expression.TrueExpression, type, out ifTrueErrors);
            success = TryAssertType(expression.FalseExpression, type, out ifFalseErrors) && success;
            if (!success)
            {
        ...
        internal static bool TryAssertRecordAsType(this IEdmRecordExpression expression, IEdmTypeReference type, out IEnumerable<EdmError> discoveredErrors)
        {
            ...
            foreach (IEdmProperty typeProperty in structuredType.StructuredDefinition().Properties())
            {
                ...
                else
                {
                    ...
                    if (!expressionProperty.Value.TryAssertType(typeProperty.Type, out recursiveErrors))
                    {
        ...
        internal static bool TryAssertCollectionAsType(this IEdmCollectionExpression expression, IEdmTypeReference type, out IEnumerable<EdmError> discoveredErrors)
        {
            ...
            {
                success = TryAssertType(element, collectionElementType, out recursiveErrors) && success;
