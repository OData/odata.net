...
namespace Microsoft.OData.Edm.Csdl.Internal.Serialization
{
    internal sealed class EdmModelCsdlSerializationVisitor : EdmModelVisitor
    {
        ...
        protected override void ProcessOperation(IEdmOperation operation)
        {
            if (operation.ReturnType != null)
            {
                bool inlineReturnType = IsInlineType(operation.ReturnType);
                this.BeginElement(operation, (IEdmOperation f) => { this.schemaWriter.WriteOperationElementHeader(f, inlineReturnType); }, f => { this.ProcessFacets(f.ReturnType, inlineReturnType); });
                if (!inlineReturnType)
                {
            ...
            {
                this.BeginElement(operation, (IEdmOperation t) => { this.schemaWriter.WriteOperationElementHeader(t, false /*Inline ReturnType*/); });
            }
        ...
        protected override void ProcessOperationParameter(IEdmOperationParameter element)
        {
            ...
            this.BeginElement(
                element, 
                (IEdmOperationParameter t) => { this.schemaWriter.WriteFunctionParameterElementHeader(t, inlineType); },
        ...
        protected override void ProcessOperationImport(IEdmOperationImport operationImport)
        {
            if (operationImport.ReturnType != null && !IsInlineType(operationImport.ReturnType))
            {
                throw new InvalidOperationException(Edm.Strings.Serializer_NonInlineFunctionImportReturnType(operationImport.Container.FullName() + "/" + operationImport.Name));
            }

            this.BeginElement(operationImport, this.schemaWriter.WriteOperationImportElementHeader);
            this.VisitOperationParameters(operationImport.Parameters);
            this.EndElement(operationImport);
        }
