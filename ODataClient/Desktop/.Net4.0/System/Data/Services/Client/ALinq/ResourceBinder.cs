    ...
    internal class ResourceBinder : DataServiceALinqExpressionVisitor
    {
        ...
        private static Expression AnalyzePredicate(MethodCallExpression mce, ClientEdmModel model)
        {
            ...
            if (inputPredicates != null)
            {
                ...
                if (!input.ContainsNonKeyPredicate)
                {
        ...
        private static bool TryAnalyzeSelectManyCollector(ResourceExpression input, Expression navPropRef, DataServiceContext context, out ResourceSetExpression result)
        {
                ...
                (sequenceMethod == SequenceMethod.Cast || sequenceMethod == SequenceMethod.OfType))
            {
                ...
                if (TryAnalyzeSelectManyCollector(input, call.Arguments[0], context, out result))
                {
                    ...
                    {
                        result = AnalyzeCast(call) as ResourceSetExpression;
                    }
                    else
                    {
                        result = AnalyzeOfType(call, context.MaxProtocolVersion) as ResourceSetExpression;
                    }
                }
                else
                {
...
#endif
                if (!ProjectionAnalyzer.Analyze(selector, sourceResourceSet, false, context))
                {
        ...
        {
            Expression obj = StripConvert(mce.Object);
            ResourceSetExpression rse = obj as ResourceSetExpression;
            if (rse == null)
            {
                return mce;
            }
        ...
        internal override Expression VisitResourceSetExpression(ResourceSetExpression rse)
        {
            ...
            if ((ResourceExpressionType)rse.NodeType == ResourceExpressionType.RootResourceSet)
            {
                ...
                return new ResourceSetExpression(rse.Type, rse.Source, rse.MemberExpression, rse.ResourceType, null /*expandPaths*/, CountOption.None, null /*customQueryOptions*/, null /*projection*/, rse.ResourceTypeAs, rse.UriVersion);
            }
        ...
        private static Expression AnalyzeCountMethod(MethodCallExpression mce)
        {
            ...
            ValidationRules.RequireCanAddCount(rse);
            rse.ConvertKeyToFilterExpression();
            rse.CountOption = CountOption.ValueOnly;
                ...
                if (ReflectionUtil.TryIdentifySequenceMethod(mce.Method, out sequenceMethod))
                {
                    switch (sequenceMethod)
                    {
                        ...
                        case SequenceMethod.OfType:
                            return AnalyzeOfType(mce, this.Model.MaxProtocolVersion);
                        case SequenceMethod.Any:
                        case SequenceMethod.All:
                        case SequenceMethod.AnyPredicate:
                            return AnalyzeAnyAll(mce, this.Model.MaxProtocolVersion);
            ...
            internal static void ValidateExpandPath(Expression input, DataServiceContext context, out string expandPath, out Version uriVersion)
            {
                ...
                if (PatternRules.MatchSingleArgumentLambda(input, out le))
                {
                    ...
                    if (pathExpression != null && PatternRules.MatchPropertyAccess(pathExpression, context, out propertyMember, out foundInstance, out propertyPath, out uriVersion))
                    {
                        ...
                        {
                            Debug.Assert(propertyPath != null && propertyPath.Count > 0, "propertyPath != null && propertyPath.Count > 0");
                            expandPath = string.Join(UriHelper.FORWARDSLASH.ToString(), propertyPath);
                            return;
                        }
                    }
                }
