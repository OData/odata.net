...
// </copyright>
// <summary>
// class reponsible for creating requests and handles responses for batch request (both sync and async)
// </summary>
//---------------------------------------------------------------------
...
namespace System.Data.Services.Client
{
    ...
    internal class BatchSaveResult : BaseSaveResult
    {
        ...
        protected override ODataRequestMessageWrapper CreateRequestMessage(string method, Uri requestUri, HeaderCollection headers, HttpStack httpStack, Descriptor descriptor)
        {
            ...
            return ODataRequestMessageWrapper.CreateBatchPartRequestMessage(this.batchWriter, args, this.RequestInfo);
        }
            ...
            using (ODataMessageWriter messageWriter = Serializer.CreateMessageWriter(batchRequestMessage, this.RequestInfo, false /*isParameterPayload*/))
            {
                ...
                if (this.Queries != null)
                {
                    foreach (DataServiceRequest query in this.Queries)
                    {
                        ...
                        ODataRequestMessageWrapper batchOperationRequestMessage = this.CreateRequestMessage(XmlConstants.HttpMethodGet, requestUri, headers, this.RequestInfo.HttpStack, null /*descriptor*/);
...
#if WINDOWS_PHONE
                        operationRequestMessage.FireWritingRequest();
#endif
...
#if WINDOWS_PHONE
                batchRequestMessage.FireWritingRequest();
#endif
            }
        ...
        private DataServiceResponse HandleBatchResponse()
        {
            ...
            try
            {
                ...
                BaseSaveResult.HandleResponse(
                    ...
                    this.batchResponseMessage.GetHeader(XmlConstants.HttpDataServiceVersion),  // responseVersion
                        ...
                        case ODataBatchReaderState.Operation:
                            ...
                            if (!insideChangeset)
                            {
                                ...
                                if (qresponse == null)
                                {
                                    ...
                                    {
                                        // This is top-level failure for SaveChanges(SaveChangesOptions.Batch) or SaveChanges(SaveChangesOptions.BatchWithIndependentOperations) operations.
        ...
        private Exception ProcessCurrentOperationResponse(ODataBatchReader batchReader, bool isChangesetOperation)
        {
            ...
            IODataResponseMessage operationResponseMessage = batchReader.CreateOperationResponseMessage();
            ...
            if (isChangesetOperation)
            {
                ...
                this.entryIndex = this.ValidateContentID(new HeaderCollection(operationResponseMessage.Headers));
...
#if WINDOWS_PHONE
            if (this.RequestInfo.HasReadingResponseEventHandlers)
            {
                InternalODataResponseMessage internalODataResponseMessage = new InternalODataResponseMessage(operationResponseMessage, true, this.RequestInfo);
                internalODataResponseMessage.FireReadingResponse(true);
                operationResponseMessage = internalODataResponseMessage;
            }
#endif
            ...
            Version responseVersion;
            string headerName = XmlConstants.HttpDataServiceVersion;
        ...
        {
            int contentID = 0;
            string contentIDTextValue;

            if (!contentHeaders.TryGetHeader(XmlConstants.HttpContentID, out contentIDTextValue) ||
                !Int32.TryParse(contentIDTextValue, NumberStyles.Integer, NumberFormatInfo.InvariantInfo, out contentID))
            {
