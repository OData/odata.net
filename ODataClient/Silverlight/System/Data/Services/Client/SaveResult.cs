...
namespace System.Data.Services.Client
{
    ...
    internal class SaveResult : BaseSaveResult
    {
        ...
        protected ContentStream CreateNonBatchChangeData(int index, ODataRequestMessageWrapper requestMessage)
        {
            ...
            {
#if WINDOWS_PHONE
                requestMessage.FireWritingRequest();
#endif
        ...
        private ODataRequestMessageWrapper CheckAndProcessMediaEntryPost(EntityDescriptor entityDescriptor)
        {
            ...
            if (type.MediaDataMember != null)
            {
                ...
                mediaRequest.AddHeadersToReset(XmlConstants.HttpContentLength);
                mediaRequest.AddHeadersToReset(XmlConstants.HttpContentType);
            }
            else
            {
                HeaderCollection headers = new HeaderCollection();
                IEnumerable<string> headersToReset = this.SetupMediaResourceRequest(headers, entityDescriptor.SaveStream, null /*etag*/);
                ...
                mediaRequest.AddHeadersToReset(headersToReset);
            }
        ...
        private ODataRequestMessageWrapper CheckAndProcessMediaEntryPut(EntityDescriptor entityDescriptor)
        {
            ...
            HeaderCollection headers = new HeaderCollection();
            IEnumerable<string> headersToReset = this.SetupMediaResourceRequest(headers, entityDescriptor.SaveStream, entityDescriptor.GetLatestStreamETag());
            ...
            mediaRequestMessage.AddHeadersToReset(headersToReset);
        ...
        private ODataRequestMessageWrapper CreateMediaResourceRequest(Uri requestUri, string method, Version version, bool sendChunked, bool applyResponsePreference, HeaderCollection headers, Descriptor descriptor)
        {
...
#if !ASTORIA_LIGHT
            ODataRequestMessageWrapper requestMessage = this.CreateRequestMessage(method, requestUri, headers, this.RequestInfo.HttpStack, descriptor);
#else
            ...
            ODataRequestMessageWrapper requestMessage = this.CreateRequestMessage(method, requestUri, headers, HttpStack.ClientHttp, descriptor);
#endif
        ...
        private IEnumerable<string> SetupMediaResourceRequest(HeaderCollection headers, DataServiceSaveStream saveStream, string etag)
        {
            ...
            // Since we are ignoring the Accept header while setting the header values,
            // we should not add it to the set of headers to reset.
            Dictionary<string, string> sourceHeaders = saveStream.Args.Headers;
            List<string> headersToReset;
            if (sourceHeaders.ContainsKey(XmlConstants.HttpAccept))
            {
                headersToReset = new List<string>(sourceHeaders.Count - 1);
                headersToReset.AddRange(sourceHeaders.Keys.Where(h => !string.Equals(h, XmlConstants.HttpRequestAccept, StringComparison.OrdinalIgnoreCase)));
            }
            else
            {
                headersToReset = sourceHeaders.Keys.ToList();
            }

            if (etag != null)
            {
                headers.SetHeader(XmlConstants.HttpRequestIfMatch, etag);
                headersToReset.Add(XmlConstants.HttpRequestIfMatch);
            }

            return headersToReset;
        ...
        private void HandleOperationException(InvalidOperationException e, IODataResponseMessage response)
        {
            ...
            if (null != response)
            {
                ...
                e = BaseSaveResult.HandleResponse(
                    this.RequestInfo,
                    statusCode,
                    response.GetHeader(XmlConstants.HttpDataServiceVersion),
                    response.GetStream,
        ...
        private void HandleOperationResponseData(IODataResponseMessage responseMsg, Stream responseStream)
        {
            ...
            Exception exception = BaseSaveResult.HandleResponse(this.RequestInfo, (HttpStatusCode)responseMsg.StatusCode, responseMsg.GetHeader(XmlConstants.HttpDataServiceVersion), () => { return responseStream; }, false/*throwOnFailure*/, out responseVersion);
        ...
        private ODataRequestMessageWrapper CreateNamedStreamRequest(StreamDescriptor namedStreamInfo)
        {
            ...
            HeaderCollection headers = new HeaderCollection();
            IEnumerable<string> headersToReset = this.SetupMediaResourceRequest(headers, namedStreamInfo.SaveStream, namedStreamInfo.GetLatestETag());
