//---------------------------------------------------------------------
// <copyright file="ODataJsonWriterEnumIntegrationTests.cs" company="Microsoft">
...
namespace Microsoft.OData.Tests.IntegrationTests.Writer.Json
{
    ...
    /// </summary>
    public class ODataJsonWriterEnumIntegrationTests
    {
        ...
        public ODataJsonWriterEnumIntegrationTests()
        {
        ...
        {
            // with model
            // write request (JsonMetadataLevel.Create method will internally use MinimalMetadata for writing request)
            var stream = new MemoryStream();
            var outputContext = CreateJsonOutputContext(stream, mediaType, false, this.userModel, setMetadataDocumentUri ? this.serviceDocumentUri : null);
            var writer = new ODataJsonWriter(outputContext, this.entitySet, this.entityType, nestedItemToWrite[0] is ODataResourceSet);
        ...
        private void WriteResponseWithModelAndValidatePayload(ODataMediaType mediaType, ODataItem[] nestedItemToWrite, string expectedPayload, bool setMetadataDocumentUri = true)
        {
            ...
            // write response
            var stream = new MemoryStream();
            var outputContext = CreateJsonOutputContext(stream, mediaType, true, this.userModel, setMetadataDocumentUri ? this.serviceDocumentUri : null);
            var writer = new ODataJsonWriter(outputContext, this.entitySet, this.entityType, nestedItemToWrite[0] is ODataResourceSet);
            WriteNestedItems(nestedItemToWrite, writer);
            ValidateWrittenPayload(stream, writer, expectedPayload);
        }
        ...
        {
            // without model, write request (JsonMetadataLevel.Create method will internally use MinimalMetadata for writing request)
            ODataMediaType mediaType = new ODataMediaType("application", "json", new KeyValuePair<string, string>(MimeConstants.MimeMetadataParameterName, MimeConstants.MimeMetadataParameterValueMinimal));

            // 1. CreateEntityContainerElementContextUri(): no entitySetName --> no context uri is output.
            // 2. but odata.type will be output because of no model. JsonMinimalMetadataTypeNameOracle.GetEntryTypeNameForWriting method: // We only write entity type names in Json if it's more derived (different) from the expected type name.
            var stream = new MemoryStream();
            var outputContext = CreateJsonOutputContext(stream, mediaType, false, null, setMetadataDocumentUri ? this.serviceDocumentUri : null);
            var writer = new ODataJsonWriter(outputContext, null, null, nestedItemToWrite[0] is ODataResourceSet);
            WriteNestedItems(nestedItemToWrite, writer);
            ValidateWrittenPayload(stream, writer, expectedPayload);
        }
        ...
        private void WriteNoMetadataResponseWithoutModelAndValidatePayload(ODataItem[] nestedItemToWrite, string expectedPayload, bool setMetadataDocumentUri = true)
        {
            ...
            ODataMediaType mediaType = new ODataMediaType("application", "json", new KeyValuePair<string, string>(MimeConstants.MimeMetadataParameterName, MimeConstants.MimeMetadataParameterValueNone));
            var stream = new MemoryStream();
            var outputContext = CreateJsonOutputContext(stream, mediaType, true, null, setMetadataDocumentUri ? this.serviceDocumentUri : null);
            var writer = new ODataJsonWriter(outputContext, null, null, nestedItemToWrite[0] is ODataResourceSet);
            WriteNestedItems(nestedItemToWrite, writer);
            ValidateWrittenPayload(stream, writer, expectedPayload);
        }
        ...
        private static ODataJsonOutputContext CreateJsonOutputContext(MemoryStream stream, ODataMediaType mediaType, bool writingResponse = true, IEdmModel userModel = null, Uri serviceDocumentUri = null)
        {
