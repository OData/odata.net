//---------------------------------------------------------------------
// <copyright file="ODataJsonWriterIntegrationTests.cs" company="Microsoft">
...
{
    public class ODataJsonWriterIntegrationTests
    {
        [Fact]
        public void ShouldBeAbleToWriteEntryWithIdOnly()
        {
            ...
            var actual = WriteJsonEntry(true, null, false, new ODataResource() { Id = new Uri("http://test.org/EntitySet('1')") }, entitySet, entityType);
        ...
        public void ShouldBeAbleToWriteEntryInRequestWithoutSpecifyingEntitySetOrMetadataDocumentUri()
        {
            ...
            var actual = WriteJsonEntry(true, null, false, new ODataResource(), entitySet, entityType);
            Assert.Equal(expected, actual);
        }
        ...
        public void ShouldBeAbleToWriteTransientEntry()
        {
            ...
            var actual = WriteJsonEntry(true, null, false, transientEntry, entitySet, entityType);
        ...
        public void WriteContainedEntitySetWithoutODataUriShouldThrow()
        {
            ...
            ODataResource entry = new ODataResource() { Properties = new[] { new ODataProperty { Name = "Key", Value = "son" }, } };
            Action writeContainedEntry = () => WriteJsonEntry(
                isRequest: false,
                serviceDocumentUri: new Uri("http://temp.org/"),
                specifySet: true,
                odataEntry: entry,
...
"}";
        ...
        public void WriteOpenEntryWithUndeclaredProperty()
        {
            ...
            var actual = WriteJsonEntryForUndeclared(
                isOpenType: true,
                writeAction: (writer) =>
                {
                    writer.WriteStart(res);
        ...
        public void WriteNonOpenEntryWithUndeclaredProperty()
        {
            ...
            Action writeEntry = () => WriteJsonEntryForUndeclared(
                isOpenType: false,
                writeAction: (writer) =>
                {
                    writer.WriteStart(res);
            ...
            writeEntry = () => WriteJsonEntryForUndeclared(
                isOpenType: false,
                writeAction: (writer) =>
                {
                    writer.WriteStart(res);
        ...
        {
            return WriteJsonEntry(isRequest, serviceDocumentUri, specifySet, odataEntry, entitySet, entityType, odataUri: null, writeAction: null, isResourceSet: false);
        }

        private static string WriteJsonEntry(bool isRequest, Uri serviceDocumentUri, bool specifySet,
        ...
        private static string WriteJsonEntryForUndeclared(bool isOpenType, Action<ODataWriter> writeAction, bool throwOnUndeclaredProperty = true)
        {
            EdmModel model = new EdmModel();
