//---------------------------------------------------------------------
// <copyright file="ODataJsonCollectionWriterTests.cs" company="Microsoft">
...
{
    public class ODataJsonCollectionWriterTests
    {
        ...
        private void WriteAndValidateSync(IEdmTypeReference itemTypeReference, ODataCollectionStart collectionStart, IEnumerable<object> items, string expectedPayload, bool writingResponse)
        {
            ...
            var outputContext = CreateJsonOutputContext(stream, this.model, writingResponse, synchronous: true);
            var collectionWriter = new ODataJsonCollectionWriter(outputContext, itemTypeReference);
        ...
        private static ODataJsonOutputContext CreateJsonOutputContext(Stream stream, IEdmModel model, bool writingResponse = true, bool synchronous = true)
        {
        ...
        /// <summary>
        /// Sets up an ODataJsonCollectionWriter,
        ...
        /// </summary>
        private async Task<string> SetupJsonCollectionWriterAndRunTestAsync(
            Func<ODataJsonCollectionWriter, Task> func,
            IEdmModel model,
