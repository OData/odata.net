...
namespace Microsoft.OData.Tests.Json
{
    ...
    public abstract class JsonWriterBaseTests
    {
      ...
      /*  public void WritesJsonElementCorrectly()
        ...
        [InlineData("text/plain")]
        [InlineData("text/html")]
        public void CorrectlyStreamsLargeStringsWithSpecialCharacters_AtDifferentPositions_ToOutput(string contentType)
        {
            int inputLength = 1024 * 1024; // 1MB
            string input = new string('a', inputLength);
            string expectedString = ExpectedOutPutStringWithSpecialCharacters(input);
            ...
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                {
                    string rawOutput = reader.ReadToEnd();
                    Assert.Equal($"\"{expectedString}\"", rawOutput);
                }
            }
        }
        ...
        private static void WriteLargeStringInChunks(TextWriter tw, string input)
        {
            // Define chunk size
            int chunkSize = 4096;
        ...
        private static void WriteLargeStringsWithSpecialCharactersInChunks(TextWriter tw, string input)
        {
            // Define chunk size
            int chunkSize = 4096;
        ...
        private static string ExpectedOutPutStringWithSpecialCharacters(string input)
        {
            string s = "";
            // Define chunk size
            int chunkSize = 4096;
