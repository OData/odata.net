//---------------------------------------------------------------------
// <copyright file="CsdlJsonSchemaParserTests.cs" company="Microsoft">
...
namespace Microsoft.OData.Edm.Tests.Csdl.Json.Parser
{
    public class CsdlJsonSchemaParserTests
    {
        private CsdlJsonSchemaParser _parser;

        public CsdlJsonSchemaParserTests()
        {
            _parser = new CsdlJsonSchemaParser(EdmConstants.EdmVersion4, CsdlSerializerOptions.DefaultOptions);
        }

        [Fact]
        public void ParseCsdlSchemaWorksAsExpectedForBasicSchema()
        {
            string json = @" {
  ""$Alias"": ""sam""
}";
            ...
            CsdlSchema schema = _parser.ParseCsdlSchema("Ns1", jsonValue, jsonPath);

            Assert.NotNull(schema);
            Assert.Equal("Ns1", schema.Namespace);
            Assert.Equal("sam", schema.Alias);
            Assert.Empty(schema.StructuredTypes);
            Assert.Empty(schema.Terms);
            Assert.Empty(schema.EntityContainers);
        }

        [Fact]
        public void ParseCsdlEntityContainerWorksAsExpectedForBasicEntityContainer()
        {
            string json = @"  {
  ""$Kind"": ""EntityContainer""
            ...
            CsdlEntityContainer entityContainer = _parser.ParseCsdlEntityContainer("default", jsonValue, jsonPath);

            Assert.NotNull(entityContainer);
            Assert.Equal("default", entityContainer.Name);
        ...
        [Fact]
        public void ParseCsdlEntityContainerThrowsForAsExpectedForBasicEntityContainer()
        {
            string json = @"  {
  ""$Kind"": ""EntityContainer""
}";
