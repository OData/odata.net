//---------------------------------------------------------------------
// <copyright file="CsdlWriterTests.Async.cs" company="Microsoft">
    ...
    ""Default"": {
      ...
      ""TestFunction"": {
        ...
        public async Task TryWriteCsdlAsyncShouldFlushAsync_Async(CsdlTarget csdlTarget, string schemaParentOpeningPartial, string schemaParentClosingPartial)
        {
            ...
            {
                Tuple<bool, IEnumerable<EdmError>> result = await CsdlWriter.TryWriteCsdlAsync(model, writer, csdlTarget).ConfigureAwait(false);
    ...
    public async Task TryWriteCsdlAsyncFromLargeModelToJson_Async()
    {
        ...
        internal static async Task WriteAndVerifyXmlAsync(IEdmModel model, string expected, CsdlTarget target = CsdlTarget.OData)
        {
            using (StringWriter sw = new StringWriter())
            {
                ...
                {
                    Tuple<bool, IEnumerable<EdmError>> result = await CsdlWriter.TryWriteCsdlAsync(model, xw, target).ConfigureAwait(false);
