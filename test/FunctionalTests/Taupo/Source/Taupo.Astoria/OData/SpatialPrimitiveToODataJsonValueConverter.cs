...
namespace Microsoft.Test.Taupo.Astoria.OData
{
    ...
    /// <summary>
    /// Default implementation of the contract to convert a spatial primitive to and OData-specific json value
    /// </summary>
    [ImplementationName(typeof(ISpatialPrimitiveToODataJsonValueConverter), "Default")]
    public class SpatialPrimitiveToODataJsonValueConverter : ISpatialPrimitiveToODataJsonValueConverter
    {
        /// <summary>
        ...
        internal static bool TryExtractMetadataTypeName(JsonObject jsonObject, out string metadataTypeName)
        {
            ...
            var metadataProperty = jsonObject.Properties.SingleOrDefault(p => p.Name == ODataConstants.JsonMetadataPropertyName);
            if (metadataProperty == null)
            {
                return false;
            }

            var metadataObject = metadataProperty.Value as JsonObject;
            if (metadataObject == null)
            {
                return false;
            }

            var typeProperty = metadataObject.Properties.SingleOrDefault(p => p.Name == ODataConstants.TypeAttributeName);
