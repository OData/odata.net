//---------------------------------------------------------------------
// <copyright file="LinqSelectExpression.cs" company="Microsoft">
...
namespace Microsoft.Test.Taupo.Query.Contracts.Linq.Expressions
{
    ...
    /// <summary>
    /// Expression node representing the Select expression in a Linq query.
    /// </summary>
    public class LinqSelectExpression : LinqQueryMethodWithLambdaExpression
    {
        internal LinqSelectExpression(QueryExpression source, LinqLambdaExpression selector, QueryType type)
            : base(source, selector, type)
        {
        }
        ...
        {
            return string.Format(CultureInfo.InvariantCulture, "{0}.Select({1})", this.Source, this.Lambda);
        }
