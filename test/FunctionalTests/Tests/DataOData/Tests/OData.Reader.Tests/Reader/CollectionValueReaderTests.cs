...
namespace Microsoft.Test.Taupo.OData.Reader.Tests.Reader
{
    ...
    public class CollectionValueReaderTests : ODataReaderTestCase
    {
        ...
        public IEnumerable<PayloadReaderTestDescriptor> CreateInvalidCollectionsWithTypeNames(bool expectedTypeWillBeUsed)
        {
            ...
            {
                testDescriptor.PayloadNormalizers.Add((tc) => tc.Format == ODataFormat.Json ? AddJsonTypeAnnotationToCollectionsVisitor.Normalize : (Func<ODataPayloadElement, ODataPayloadElement>)null);
            }
    ...
    /// <summary>
    /// Visitor for generating Json context uri annotations to replace any expected type annotations (that
    /// may have been generated by the Json fixup)
    /// </summary>
    internal class ReplaceExpectedTypeWithContextUriVisitor : ODataPayloadElementVisitorBase
    {
        ...
        private void ReplaceExpectedTypeAnnotationIfRootElement(ODataPayloadElement payloadElement)
        {
            if (this.payloadElementStack.Count == 2)
            {
                ...
                {
                    payloadElement.WithContextUri(JsonODataPayloadElementExtensions.BuildContextUri(payloadElement.ElementType, JsonConstants.DefaultMetadataDocumentUri.OriginalString, expectedTypeAnnotation));
    ...
    #region AddJsonTypeAnnotationToCollectionsVisitor
    /// <summary>
    /// Visitor for adding a Json type property annotation to collection values.
    /// </summary>
    public class AddJsonTypeAnnotationToCollectionsVisitor : ODataPayloadElementVisitorBase
    {
        private readonly Stack<ODataPayloadElement> payloadElementStack = new Stack<ODataPayloadElement>();

        public static ODataPayloadElement Normalize(ODataPayloadElement payloadElement)
        {
            new AddJsonTypeAnnotationToCollectionsVisitor().Recurse(payloadElement);
            return payloadElement;
        }
        ...
        private void AddTypeAnnotation(ODataPayloadElement payloadElement, string typeName)
        {
            ...
            {
                propertyInstance.WithPropertyAnnotation(JsonConstants.ODataTypeAnnotationName, typeName);
            }
        }
    }
    #endregion
