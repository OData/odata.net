...
namespace Microsoft.Test.OData.TDD.Tests.Common
{
    ...
    public static class TestUtils
    {
        ...
        {
            Assert.IsTrue(referencedModels[0] != null, "referencedModels[0] != null");
...
<edmx:Edmx Version=""4.0"" xmlns:edmx=""http://docs.oasis-open.org/odata/ns/edmx"">
  ...
  <edmx:DataServices>
    <Schema Namespace=""MainModel_NS1"" xmlns=""http://docs.oasis-open.org/odata/ns/edm"">
            ...
            Assert.AreEqual(0, errors.Count(), "should be zero error.");
            return null;
        }
        ...
        {
            Assert.AreEqual(nameOfContainerToExtend + "_sub", referencedModels[0].EntityContainer.Name, "the container name of '" + nameOfContainerToExtend + "' will be used by main model, so the container name in referenced model must have been appended with '_sub'.");
            string mainModelxml = @"<?xml version=""1.0"" encoding=""utf-16""?>
<edmx:Edmx Version=""4.0"" xmlns:edmx=""http://docs.oasis-open.org/odata/ns/edmx"">
  <edmx:Reference Uri=""http://host/schema/Location.xml"">
    <edmx:Include Namespace=""Test.Chh"" Alias=""NO_Alias"" />
  ...
  <edmx:DataServices>
    <Schema Namespace=""{0}"" xmlns=""http://docs.oasis-open.org/odata/ns/edm"">  
        ...
        {
            Assert.IsNotNull(collectionValue1);
            Assert.IsNotNull(collectionValue2);
            Assert.AreEqual(collectionValue1.TypeName, collectionValue2.TypeName);
            var itemsArray1 = collectionValue1.Items.OfType<object>().ToArray();
            var itemsArray2 = collectionValue2.Items.OfType<object>().ToArray();

            Assert.AreEqual(itemsArray1.Length, itemsArray2.Length);
            for (int i = 0; i < itemsArray1.Length; i++)
            {
                ...
                {
                    Assert.AreEqual(itemsArray1[i], itemsArray2[i]);
                }
            }
        }
        ...
        {
            Assert.IsNotNull(complexValue1);
            Assert.IsNotNull(complexValue2);
            Assert.AreEqual(complexValue1.TypeName, complexValue2.TypeName);
        ...
        {
            Assert.IsNotNull(odataProperty1);
            Assert.IsNotNull(odataProperty2);
            Assert.AreEqual(odataProperty1.Name, odataProperty2.Name);
