                ...
                while (batchReader.Read())
                {
                    switch (batchReader.State)
                    {
                        case ODataBatchReaderState.Operation:
                                ...
                                using (ODataMessageReader innerMessageReader = new ODataMessageReader(operationMessage, new ODataMessageReaderSettings(), this.userModel))
                                {
                                    ...
                                    while (reader.Read())
                                    {
                                        if (reader.State == ODataReaderState.EntryEnd)
                                        {
                                            ...
                                            Assert.AreEqual(10, entry.Properties.Single(p => p.Name == "WebId").Value);
                                            Assert.AreEqual("WebSingleton", entry.Properties.Single(p => p.Name == "Name").Value);
                                        }
                                    }
                                }
                            }
                            ...
                            {
                                Assert.IsTrue(204 == operationMessage.StatusCode || 500 == operationMessage.StatusCode);
                            }
                            break;
                    }
                }
