...
namespace Microsoft.Test.OData.TDD.Tests.Reader.JsonLight
{
    ...
    public class ODataJsonLightDeltaReaderTests
    {
        ...
        private IEnumerable<Tuple<ODataItem, ODataDeltaReaderState>> ReadItem(string payload, IEdmModel model = null, IEdmNavigationSource navigationSource = null, IEdmEntityType entityType = null)
        {
            ...
            foreach (var tuple in tuples)
            {
                switch (tuple.Item2)
                {
                    case ODataDeltaReaderState.DeltaFeedStart:
                        ...
                        Assert.IsNotNull(deltaFeed);
                        if (deltaFeed.Count.HasValue)
                        {
                            Assert.AreEqual(deltaFeed.Count, feed.Count);
                        }
                        break;
                    case ODataDeltaReaderState.FeedEnd:
                        Assert.IsNotNull(tuple.Item1 as ODataDeltaFeed);
                        if (nextLink != null)
                        {
                            Assert.AreEqual(nextLink, ((ODataDeltaFeed)tuple.Item1).NextPageLink);
                        }
                        if (feedDeltaLink != null)
                        {
                            Assert.AreEqual(feedDeltaLink, ((ODataDeltaFeed)tuple.Item1).DeltaLink);
                        }
                        break;
                    case ODataDeltaReaderState.DeltaEntryStart:
                        Assert.IsTrue(tuple.Item1 is ODataEntry);
                        break;
                    case ODataDeltaReaderState.DeltaEntryEnd:
                        var deltaEntry = tuple.Item1 as ODataEntry;
                        Assert.IsNotNull(deltaEntry);
                        Assert.IsNotNull(deltaEntry.Id);
                        if (this.IdEqual(deltaEntry.Id, customerUpdated.Id))
                        {
                            Assert.IsTrue(PropertiesEqual(deltaEntry.Properties, customerUpdated.Properties));
                        }
                        else if (this.IdEqual(deltaEntry.Id, order10643.Id))
                        {
                            Assert.IsTrue(this.PropertiesEqual(deltaEntry.Properties, order10643.Properties));
                        }
                        else
                        {
                            Assert.Fail("Invalid id read.");
                        }
                        break;
                    case ODataDeltaReaderState.DeltaDeletedEntry:
                        var deltaDeletedEntry = tuple.Item1 as ODataDeltaDeletedEntry;
                        Assert.IsNotNull(deltaDeletedEntry);
                        Assert.IsTrue(deltaDeletedEntry.Id.EndsWith(customerDeleted.Id));
                        Assert.AreEqual(deltaDeletedEntry.Reason, customerDeleted.Reason);
                        break;
                    case ODataDeltaReaderState.DeltaLink:
                        var deltaLink = tuple.Item1 as ODataDeltaLink;
                        Assert.IsNotNull(deltaLink);
                        Assert.IsTrue(this.IdEqual(deltaLink.Source, linkToOrder10645.Source));
                        Assert.AreEqual(deltaLink.Relationship, linkToOrder10645.Relationship);
                        Assert.IsTrue(this.IdEqual(deltaLink.Target, linkToOrder10645.Target));
                        break;
                    case ODataDeltaReaderState.DeltaDeletedLink:
                        var deltaDeletedLink = tuple.Item1 as ODataDeltaDeletedLink;
                        Assert.IsNotNull(deltaDeletedLink);
                        Assert.IsTrue(this.IdEqual(deltaDeletedLink.Source, linkToOrder10643.Source));
                        Assert.AreEqual(deltaDeletedLink.Relationship, linkToOrder10643.Relationship);
                        Assert.IsTrue(this.IdEqual(deltaDeletedLink.Target, linkToOrder10643.Target));
                        break;
                    default:
