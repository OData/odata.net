...
namespace Microsoft.Test.OData.TDD.Tests
{
    ...
    {
        private const string TestNameSpace = "Microsoft.Test.OData.TDD.Tests";
        ...
        // TODO: Complete after Singleton Freature
        [TestMethod]
        [Ignore]
        ...
        public void MissmatchContextUrlInRequest()
        {
            foreach (ODataFormat mimeType in mimeTypes)
            {
                ...
                payload = payload.Replace("$metadata#People/Microsoft.Test.OData.TDD.Tests.Employee", "WrongURL");
                this.ReadPayload(payload, contentType, model, omReader =>
                {
                    var reader = omReader.CreateODataFeedReader(this.peopleSet, this.employeeType);
                    while (reader.Read()) { }
