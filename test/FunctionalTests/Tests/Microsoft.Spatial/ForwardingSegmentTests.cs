...
namespace DataSpatialUnitTests.Tests
{
    ...
    public class ForwardingSegmentTests
    {
        ...
        private void ExceptionCallsResetForForwardCallsWithArgs(TracingPipe pipe)
        {
            ...
            StringAssert.Contains(ex.StackTrace, "DoWhenNotCall", "Lost the original stack trace");
        ...
        private void AssertResetCalledLastOnCurrentAndDownstream()
        {
            ...
            Assert.AreNotEqual(0, this.currentNode.Calls.Count, "No calls on currentNode");
            Assert.AreNotEqual(0, this.downstreamSegment.Calls.Count, "No calls on downstreamSegment");
            
            // the last call should be a reset call
            Assert.AreEqual(this.resetCall, this.currentNode.Calls[this.currentNode.Calls.Count-1]);
            Assert.AreEqual(this.resetCall, this.downstreamSegment.Calls[this.downstreamSegment.Calls.Count-1]);
            
            // we should have only gotten 1 reset call each
            Assert.AreEqual(1, this.CountCallsToAMethod(this.currentNode.Calls, resetCall), "go more than one call to reset");
            Assert.AreEqual(1, this.CountCallsToAMethod(this.downstreamSegment.Calls, resetCall), "go more than one call to reset");
        }
