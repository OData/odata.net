...
namespace Microsoft.OData.Tests.UriParser.Parsers
{
    public class UriQueryExpressionParserTests
    {
        ...
        public void ParseCollectionWithIntegers()
        {
            ...
            Assert.NotNull(token);
            var collectionToken = Assert.IsType<CollectionLiteralToken>(token);
            Assert.Equal(5, collectionToken.Count);
        }
        ...
        public void ParseCollectionWithStrings(string input)
        {
            ...
            Assert.NotNull(token);
            var collectionToken = Assert.IsType<CollectionLiteralToken>(token);
            Assert.Equal(3, collectionToken.Count());
        }
        ...
        public void ParseCollectionWithMixedTypes()
        {
            ...
            var items = collectionToken.ToList();
        ...
        public void ParseNestedCollections()
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                Assert.IsType<CollectionLiteralToken>(collectionToken.ElementAt(i));
            }
        }
        ...
        public void ParseDeeplyNestedCollections()
        {
            ...
            Assert.NotNull(token);
            var collectionToken = Assert.IsType<CollectionLiteralToken>(token);
            CollectionLiteralToken deep1Collection = Assert.IsType<CollectionLiteralToken>(Assert.Single(collectionToken));
            CollectionLiteralToken deep2Collection = Assert.IsType<CollectionLiteralToken>(Assert.Single(deep1Collection));
            Assert.Equal(2, deep2Collection.Count);
        }
        ...
        public void ParseEmptyMap()
        {
            QueryToken token = this.testSubject.ParseExpressionText("{}");
            Assert.NotNull(token);
            var mapToken = Assert.IsType<ResourceLiteralToken>(token);
            Assert.Empty(mapToken);
        }
        ...
        public void ParseMapWithNestedMap()
        {
            ...
            Assert.Single(resourceLiteralToken);
        }
        ...
        public void ParseMapWithMixedValueTypes()
        {
            ...
            Assert.NotNull(token);
            var resourceLiteralToken = Assert.IsType<ResourceLiteralToken>(token);
            Assert.Equal(5, resourceLiteralToken.Count());
        }
        ...
        public void ParseMapWithWhitespace()
        {
            ...
            Assert.NotNull(token);
            var resourceLiteralToken = Assert.IsType<ResourceLiteralToken>(token);
            Assert.Equal(2, resourceLiteralToken.Count());
        }
        ...
        public void ParseComplexNestedStructure()
        {
            ...
            KeyValuePair<string, QueryToken> userPair = Assert.Single(rootMap);
            ...
            Assert.Equal(2, userMap.Count());

            var namePair = userMap.First(kvp => kvp.Key == "Na\"me"); // escaped
            var nameToken = Assert.IsType<LiteralToken>(namePair.Value);
            Assert.Equal("John", nameToken.Value);

            var contactsPair = userMap.First(kvp => kvp.Key == "Contacts");
            ...
            QueryToken contactItem = Assert.Single(contactsCollection);
            ...
            Assert.Equal(2, contactMap.Count());

            var typePair = contactMap.First(kvp => kvp.Key == "Type");
            Assert.Equal("Type", typePair.Key);
            var typeToken = Assert.IsType<LiteralToken>(typePair.Value);
            Assert.Equal("Email", typeToken.Value);

            var valuePair = contactMap.First(kvp => kvp.Key == "Value");
        ...
        public void ParseCollectionWithLargeNumbers()
        {
            ...
            Assert.NotNull(token);
            var collectionToken = Assert.IsType<CollectionLiteralToken>(token);
            Assert.Equal(2, collectionToken.Count());
        }
