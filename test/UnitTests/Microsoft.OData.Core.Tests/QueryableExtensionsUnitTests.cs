namespace Microsoft.OData.Core.Tests
{
    ...
    public sealed class QueryableExtensionsUnitTests
    {
        ...
        {
            var source = new EnumerableQuery2<short>(new short[] { 1, 2, 3, 4 });
            ...
            /*var source = new EnumerableQuery2<int>(new int[] { 1, 2, 3, 4 });

            var call = System.Linq.Expressions.Expression.Call(
                null,
                new Func<IQueryable<int>, double>(Queryable.Average).Method,
                source.Expression);*/
        ...
        public class EnumerableQuery2<T> : EnumerableQuery<T>
        {
            public EnumerableQuery2(IEnumerable<T> enumerable)
                : base(enumerable)
            {
        ...
        internal sealed class EnumerableRewriter : ExpressionVisitor
        {
            ...
            {
                if (c.Value is EnumerableQuery2<short> sq)
                {
            ...
            private static MethodInfo FindMethod(Type type, string name, ReadOnlyCollection<Expression> args, Type[]? typeArgs)
            {
                ...
                {
                    if (!en.MoveNext())
                        throw new NotImplementedException("OTOD");
                    do
                    {
    ...
    public static class QueryableExtensions
    {
        ...
        private static double Average(this IEnumerable<short> source)
        {
