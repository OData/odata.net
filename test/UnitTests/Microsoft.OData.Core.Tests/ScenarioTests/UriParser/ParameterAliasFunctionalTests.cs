    ...
    public class ParameterAliasFunctionalTests
    {
        ...
        public void ParsePath_AliasInFunctionImport_DateTimeOffsetPromote()
        {
            ParseUriAndVerify(
                ...
                (oDataPath, filterClause, orderByClause, selectExpandClause, aliasNodes) =>
                {
                    OperationImportSegment segment = oDataPath.LastSegment.ShouldBeOperationImportSegment(HardCodedTestModel.GetFunctionImportForGetPersonByDTO());
                    OperationSegmentParameter parameter = Assert.Single(segment.Parameters);
                    parameter.ShouldHaveParameterAliasNode("dto", "@p1", EdmCoreModel.Instance.GetDateTimeOffset(false));
                    ConvertNode convertNode = aliasNodes["@p1"].ShouldBeConvertQueryNode(EdmPrimitiveTypeKind.DateTimeOffset);
                    convertNode.Source.ShouldBeConstantQueryNode(new DateOnly(1997, 12, 12));
                });
        }
        ...
        public void ParsePath_AliasInFunctionImport_NullAsValue_ThrowsIfTypeIsNonNullable()
        {
            Action parse = () => ParseUriAndVerify(
                ...
                (oDataPath, filterClause, orderByClause, selectExpandClause, aliasNodes) => { });
            parse.Throws<ODataException>(Error.Format(SRResources.LiteralBinder_CannotBindNullLiteralToNonNullableType, "null", EdmCoreModel.Instance.GetDecimal(false).FullName()));
        }
        ...
        public void ParsePath_AliasInUnboundFunction_QuotesWithinDoubleQuotedStringsInJsonArray(string parameterValue)
        {
            ParseUriAndVerify(
                ...
                (oDataPath, filterClause, orderByClause, selectExpandClause, aliasNodes) =>
                {
                    ...
                    Assert.Equal(3, colConstNode.Items.Count);
        ...
        public void ParsePath_AliasInUnboundFunction_WithinDollarRootPath()
        {
            ParseUriAndVerify(
                ...
                (oDataPath, filterClause, orderByClause, selectExpandClause, aliasNodes) =>
                {
                    ...
                    RootPathNode path1 = colRootExprNode.Collection.ElementAt(0);
                    ...
                    RootPathNode path2 = colRootExprNode.Collection.ElementAt(1);
