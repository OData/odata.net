...
public class SearchQueryTests : EndToEndTestBase<SearchQueryTests.TestsStartup>
{
    ...
    public SearchQueryTests(TestWebApplicationFactory<TestsStartup> fixture)
        : base(fixture)
    {
    ...
    {
        List<ODataResource> details = await TestsHelper.QueryResourceSetFeedAsync("ProductDetails?$search=(drink OR snack) AND (suger OR sweet) AND NOT \"0\"", mimeType);
    ...
    {
        List<ODataResource> details = await TestsHelper.QueryResourceSetFeedAsync("ProductDetails?$search=NOT (drink OR snack)", mimeType);
        if (!mimeType.Contains(MimeTypes.ODataParameterNoMetadata))
        {
    ...
    {
        List<ODataResource> details = await TestsHelper.QueryResourceSetFeedAsync("ProductDetails?$search=snack sweet", mimeType);
        if (!mimeType.Contains(MimeTypes.ODataParameterNoMetadata))
        {
    ...
    {
        List<ODataResource> details = await TestsHelper.QueryResourceSetFeedAsync("ProductDetails?$search=snack NOT sweet", mimeType);
        if (!mimeType.Contains(MimeTypes.ODataParameterNoMetadata))
        {
            Assert.Equal(2, details.Count);
        }
    ...
    {
        List<ODataResource> details = await TestsHelper.QueryResourceSetFeedAsync("ProductDetails?$search=snack OR drink AND soft AND NOT \"0\"", mimeType);
        if (!mimeType.Contains(MimeTypes.ODataParameterNoMetadata))
        {
    ...
    {
        List<ODataResource> details = await TestsHelper.QueryResourceSetFeedAsync("ProductDetails?$filter=contains(Description,'drink')&$search=suger OR spicy NOT \"0\"&$select=Description", mimeType);
        if (!mimeType.Contains(MimeTypes.ODataParameterNoMetadata))
        {
            Assert.Equal(2, details.Count);
    ...
    {
        List<ODataResource> entries = await TestsHelper.QueryResourceSetFeedAsync("ProductDetails?$search=suger OR sweet&$orderby=ProductName&$expand=Reviews", mimeType);
        if (!mimeType.Contains(MimeTypes.ODataParameterNoMetadata))
        {
