...
public class FilterQueryTests : EndToEndTestBase<FilterQueryTests.TestsStartup>
{
    ...
    public FilterQueryTests(TestWebApplicationFactory<TestsStartup> fixture) : base(fixture)
    {
    ...
    {
        // Arrange & Act
        var entries = await TestsHelper.QueryResourceSetFeedAsync("People?$filter=Emails/$count lt 2", mimeType);
    ...
    {
        // Arrange & Act
        var entries = await TestsHelper.QueryResourceSetFeedAsync("Products?$filter=CoverColors/$count lt 2", mimeType);
        var details = entries.Where(r => r != null && r.Id != null).ToList();
    ...
    {
        // Arrange & Act
        var entries = await TestsHelper.QueryResourceSetFeedAsync("People?$filter=Addresses/$count eq 2", mimeType);
        var details = entries.Where(r => r != null && r.Id != null).ToList();
    ...
    {
        // Arrange & Act
        var entries = await TestsHelper.QueryResourceSetFeedAsync("Customers?$filter=Orders/$count lt 2", mimeType);
        var details = entries.Where(r => r != null && r.Id != null).ToList();
