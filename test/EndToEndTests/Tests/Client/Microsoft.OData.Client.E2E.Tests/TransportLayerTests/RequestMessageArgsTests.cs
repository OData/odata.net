...
namespace Microsoft.OData.Client.E2E.Tests.TransportLayerTests
{
    public class RequestMessageArgsTests : EndToEndTestBase<RequestMessageArgsTests.TestsStartup>
    {
        ...
        public async Task AddingCustomHeaderToRequest_ShouldSucceed()
        {
            ...
            this.UpdateContext(configureRequest);
        ...
        [Fact]
        public async Task ChangingMergeToPatch_ShouldUpdateRequestMethod()
        {
            Func<DataServiceClientRequestMessageArgs, DataServiceClientRequestMessage> configureRequest =
                args =>
                {
                    ...
                    {
                        var newArgs = new DataServiceClientRequestMessageArgs("PATCH", args.RequestUri, true, args.Headers);

                        // Ensure compatibility with V3
                        newArgs.Headers.Remove("DataServiceVersion");
                        newArgs.Headers.Add("DataServiceVersion", "3.0");

                        return new HttpClientRequestMessage(newArgs);
                    }
            ...
            var query = _context.Products.Take(1) as DataServiceQuery<ClientEndToEndModel.Product>;
            var product = (await query.ExecuteAsync()).Single();

            this.lastRequestMessage = null;
            product.Description = "New Description " + Guid.NewGuid();
        ...
        private Container UpdateContext(Func<DataServiceClientRequestMessageArgs, DataServiceClientRequestMessage> onMessageCreate)
        {
            _context.SendingRequest2 += (obj, args) => this.lastRequestMessage = args.RequestMessage;
            _context.Configurations.RequestPipeline.OnMessageCreating = onMessageCreate;
