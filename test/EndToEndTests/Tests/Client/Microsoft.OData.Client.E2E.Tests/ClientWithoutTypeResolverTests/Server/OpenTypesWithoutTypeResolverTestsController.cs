...
namespace Microsoft.OData.Client.E2E.Tests.ClientWithoutTypeResolverTests.Server
{
    public class OpenTypesWithoutTypeResolverTestsController : ODataController
    {
        ...
        [EnableQuery]
        [HttpGet("odata/Row({key})")]
        public IActionResult Row([FromRoute] Guid key)
        {
            var row = OpenTypesServiceDataSource.Row.SingleOrDefault(a=>a.Id == key);
        ...
        {
            var r = OpenTypesServiceDataSource.Row.SingleOrDefault(a => a.Id == key);
