  ...
  <edmx:DataServices xmlns:m=""http://docs.oasis-open.org/odata/ns/metadata"">
    <Schema Namespace=""NS1"" xmlns=""http://docs.oasis-open.org/odata/ns/edm"">
      ...
      <EntityContainer Name=""C1"">
        ...
        var exception1 = Assert.Throws<ArgumentNullException>(() => CsdlWriter.TryWriteCsdl(null, XmlWriter.Create(new StringWriter()), out errors));
        Assert.Equal("Value cannot be null. (Parameter 'model')", exception1.Message);

        var exception2 = Assert.Throws<ArgumentNullException>(() => CsdlWriter.TryWriteCsdl(model, (XmlWriter)null, out errors));
