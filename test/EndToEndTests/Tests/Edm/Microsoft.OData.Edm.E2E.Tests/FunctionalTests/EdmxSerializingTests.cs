  ...
  <edmx:DataServices xmlns:m=""http://docs.oasis-open.org/odata/ns/metadata"">
    <Schema Namespace=""NS1"" xmlns=""http://docs.oasis-open.org/odata/ns/edm"">
      ...
      <EntityContainer Name=""C1"">
        ...
        CsdlReader.TryParse(XmlReader.Create(new StringReader(inputText)), out IEdmModel model, out IEnumerable<EdmError> errors);

        Assert.Throws<ArgumentNullException>(() => CsdlWriter.TryWriteCsdl(null, XmlWriter.Create(new StringWriter()), CsdlTarget.EntityFramework, out errors));
        Assert.Throws<ArgumentNullException>(() => CsdlWriter.TryWriteCsdl(model, null, CsdlTarget.EntityFramework, out errors));
    }
}
