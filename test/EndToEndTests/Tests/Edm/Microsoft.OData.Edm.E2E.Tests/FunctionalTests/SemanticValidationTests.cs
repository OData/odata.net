...
<EntitySet Name=""Entity1_a"" EntityType=""Bork.Entity1"" />
    ...
    public void ValidateOpenComplexTypeSupported()
    {
        ...
        foreach (var v in new[] { Microsoft.OData.Edm.EdmConstants.EdmVersionLatest })
        {
            ...
            Assert.Equal(0, errors.Count(), "No error should be returned");
        }
    }
    ...
    public void ValidateAssociationNameSimpleIdentifier()
    {
        ...
        Assert.Equal(0, parsedModelErrors.Count(), "No error should be returned.");
        Assert.Equal(constructedModelErrors.Count(), parsedModelErrors.Count(), "Validation results between constructed models and parsed models shold be no different.");
    }
    ...
    public void ValidateValidator()
    {
        ...
        {
            Assert.True(e is ArgumentNullException, "Correct exception type");
        }
    }
...
<Schema Namespace=""Hot"" Alias=""Fuzz"" xmlns:Bogus=""http://bogus.com/schema"" xmlns=""http://docs.oasis-open.org/odata/ns/edm"">
  ...
  <EntityContainer Name=""DefaultContainer"">
    ...
    public void ValidateIEdmCheckableClasses()
    {
        ...
        Assert.True(expectedEdmCheckableTypes.Except(actualEdmCheckableTypes).Count() == 0, "Actual IEdmCheckable classes are diffrent from the list of the expected results.");
    }
    ...
    public void VerifyIsBadExtensionMethod()
    {
        ...
        Assert.True(testEdmCheckableElement.IsBad(), "The IsBad method should return true when an element is invalid.");

        testEdmCheckableElement = new ValidationTestModelBuilder.TestEdmCheckableElement(false);
        Assert.False(testEdmCheckableElement.IsBad(), "The IsBad method should return false when an element is valid.");
        ...
        Assert.True(testEdmElementWithAssociation.IsBad(), "The IsBad method should return true when an associated element is invalid.");

        testEdmElementWithAssociation.AssociatedProperty = new ValidationTestModelBuilder.TestEdmAssociatedCheckableElement(false);
        Assert.False(testEdmElementWithAssociation.IsBad(), "The IsBad method should return false only when the element and all of its associated elements are valid.");
    }
    ...
    public void VerifyIsBadExtensionMethodForPolymorphicVariable()
    {
        foreach (bool booleanValue in new Boolean[] { true, false })
        {
            ...
            Assert.Equal(b1, b2, "The IsBad method call on the same object should return the same result.");
        }
    }
    ...
    {
        Assert.False((new ValidationTestModelBuilder.TestEdmAssociatedCheckableNullErrorElement()).IsBad(), "IsBad should be true if if the null Error case means no error.");
    }
    ...
    public void VerifyValidatorForNullError()
    {
        ...
        Assert.False(edmError.Any(), "The number of EdmErrors should be 0 if the null Error case means no error.");
    }
    ...
    public void TestOperationWithBadReturnType()
    {
        ...
        IEnumerable<EdmError> errors;
        Assert.False(m.Validate(out errors), "m.Validate(out errors)");
        Assert.Equal(1, errors.Count(), "errors.Count()");
        Assert.True(errors.Any(e => e.ErrorCode == EdmErrorCode.TypeMustNotHaveKindOfNone), "EdmErrorCode.TypeMustNotHaveKindOfNone");
        ...
        Assert.Equal(1, errors.Count(), "Error generated on serialization");
    }
...
@"<Schema Namespace=""foobaz"" Alias=""alias3"" xmlns=""http://docs.oasis-open.org/odata/ns/edm"">
    <ComplexType Name=""Address"">
        ...
        Assert.NotNull(person, "Invalid complex type qualifier.");
        Assert.Equal("foo.Name", person.FindProperty("Name").Type.FullName(), "Invalid type full name.");
        Assert.Equal("bar.Number", person.FindProperty("Number").Type.FullName(), "Invalid type full name.");
        Assert.Equal("foobaz.Address", person.FindProperty("Address").Type.FullName(), "Invalid type full name.");
    }
...
";
    ...
    public void OneWayNavigationPropertyMultipleMappingTest()
    {
        ...
        Assert.True(!edmErrors.Any(), @"Enable multiple mappings for the same (silent) navigation property");
    ...
    public void AnnotationNameWhichIsNotSimpleIdentifierShouldPassValidation()
    {
        ...
        Assert.Equal(1, model.DirectValueAnnotations(fredFlintstone).Count(), "Wrong # of Annotations on {0}.", fredFlintstone);

        IEnumerable<EdmError> errors;
        model.Validate(out errors);
        Assert.Equal(0, errors.Count(), "Model expected to be valid.");
    }
    ...
    public void ValidationShouldFailOnNonXmlValidAnnotation()
    {
        ...
        IEnumerable<EdmError> errors;
        model.Validate(out errors);
        Assert.Equal(1, errors.Count(), "Model expected to be invalid.");
        EdmError error = errors.Single();
        Assert.Equal(error.ErrorCode, EdmErrorCode.InvalidName, "Unexpected error code");
        Assert.Equal(error.ErrorMessage, "The specified name is not allowed: '-content-version'.", "Unexpected error message");
    }
    ...
    public void ValidationShouldFailOnInvalidNavigationBindingPath()
    {
        ...
        IEnumerable<EdmError> errors;
        model.Validate(out errors);
        Assert.Equal(4, errors.Count(), "Model expected to be invalid.");
    ...
    public void SchemaWriterShouldFailWithGoodMessageWhenWritingInvalidXml()
    {
        ...
        {
            Assert.Equal(e.Message, "Invalid name character in '-contentversion'. The '-' character, hexadecimal value 0x2D, cannot be included in a name.", "Expected exception message did not match.");
        }
