...
public class VersioningTreeTests : EdmLibTestCaseBase
{
    ...
    public void VersioningTreeBasicInsertion()
    {
        ...
        Assert.Equal(OneHundred, tree.RightChild.RightChild, "Tree nodes EQ");
        ...
        Assert.Equal(Sixty, tree.LeftChild.LeftChild, "Tree nodes EQ");
        ...
        Assert.Null(tree.LeftChild.RightChild.LeftChild.RightChild, "Null child");
        VerifyKey(tree.LeftChild.RightChild.RightChild, 85);
        VerifyKey(tree.RightChild, 100);
        VerifyKey(tree.RightChild.LeftChild, 96);
        Assert.Null(tree.RightChild.LeftChild.LeftChild, "Null child");
        VerifyKey(tree.RightChild.LeftChild.RightChild, 98);
        VerifyKey(tree.RightChild.RightChild, 110);
        VerifyLeaf(tree.RightChild.RightChild);

        Assert.Equal(Fifty, tree.LeftChild.LeftChild, "Tree nodes EQ");
        Assert.Equal(OneHundred, tree.RightChild, "Tree nodes EQ");
        ...
        Assert.Equal(Sixty, tree.LeftChild, "Tree nodes EQ");
        ...
        VerifyLeaf(tree.LeftChild.LeftChild);
        Assert.Null(tree.LeftChild.RightChild, "Null child");
        VerifyKey(tree.RightChild, 100);
        VerifyKey(tree.RightChild.LeftChild, 95);
        VerifyLeaf(tree.RightChild.LeftChild);
        Assert.Null(tree.RightChild.RightChild, "Null child");
    ...
    public void VersioningTreeAscendingInsertion()
    {
        ...
        {
            Assert.Equal(i * 10, tree.GetValue(i, CompareIntegers), "Node value");
        }
        ...
        for (int i = 0; i < treeSize; i++)
        {
            Assert.Equal(i * 2, tree.GetValue(i, CompareIntegers), "Node value");
        }
    }
    ...
    {
        Assert.Equal(expectedHeight, tree.Height, "Node height");
    }

    private void VerifyKey(VersioningTree<int, int> tree, int expectedKey)
    {
        Assert.Equal(expectedKey, tree.Key, "Node key");
    }

    private void VerifyLeaf(VersioningTree<int, int> tree)
    {
        Assert.Null(tree.LeftChild, "Leaf child");
        Assert.Null(tree.RightChild, "Leaf child");
    }
}
