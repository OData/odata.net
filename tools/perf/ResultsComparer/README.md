...
This is a tool for comparison performance reports in order to quickly find regressions and improvements.
...
Sample: compare the results stored in `C:\results\BenchmarkDotNetReportBefore.json` vs `C:\results\BenchmarkDotNetReportAfter` using `1%` threshold

```cmd
cd src/ResultsComparer

dotnet run -- --type bdn --base "C:\results\windows" --diff "C:\results\ubuntu" --threshold 1%
```

## Comparing BencharmkDotNet benchmarks

**Note**: the tool supports only `*full.json` results exported by [BenchmarkDotNet](https://benchmarkdotnet.org) and those exported by [Microsoft.Crank](https://github.com/dotnet/crank) when running
BenchmarkDotNet benchmaks.
...
## Comparing Visual Studio Profiler reports

The tool currently supports comparing data generated by the [.NET Objec Allocation Tracker](https://devblogs.microsoft.com/visualstudio/net-object-allocation-tool-performance/) in the Visual Studio Profiler.
...
- Select the rows you want to copy. To select the entire table, click the first row then scroll down and click the last row while holding down `Ctrl+A`
...
With the files ready, you can run the command. Currently we support data from the **Allocations** tab and **Functions** tab:
...
This is the **Functions** tab of the profiler. This report show the number and size of allocations for each method. The methods are grouped by namesace, make sure to expand the namespaces you are interested in, otherwise their methods will not be included in the report when you copy-paste it.
