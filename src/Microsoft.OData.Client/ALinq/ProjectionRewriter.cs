...
namespace Microsoft.OData.Client
{
    ...
    internal class ProjectionRewriter : ALinqExpressionVisitor
    {
        ...
        internal static LambdaExpression TryToRewrite(LambdaExpression le, ResourceExpression source, ClientEdmModel model)
        {
            ...
            if (!ResourceBinder.PatternRules.MatchSingleArgumentLambda(le, out le) ||  // can only rewrite single parameter Lambdas.
                ClientTypeUtil.TypeOrElementTypeIsEntity(model, le.Parameters[0].Type) || // only attempt to rewrite if lambda parameter is not an entity type
