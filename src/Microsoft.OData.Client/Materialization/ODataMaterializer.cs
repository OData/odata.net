    ...
    internal abstract class ODataMaterializer : IDisposable
    {
            ...
            MaterializerCache materializerCache)
        {
            ...
            try
            {
                ...
                if (payloadKind == ODataPayloadKind.Resource || payloadKind == ODataPayloadKind.ResourceSet)
                {
                    ...
                    {
                        throw DSClient.Error.InvalidOperation(Error.Format(SRResources.Materializer_InvalidNonEntityType, materializerType.FullName));
                    }
                ...
                else
                {
                    switch (payloadKind)
                    {
                        ...
                        case ODataPayloadKind.IndividualProperty:
                            ...
                            {
                                throw DSClient.Error.InvalidOperation(Error.Format(SRResources.Materializer_InvalidEntityType, materializerType.FullName));
                            }
                        ...
                        default:
                            throw DSClient.Error.InvalidOperation(Error.Format(SRResources.Materializer_InvalidResponsePayload, XmlConstants.DataWebNamespace));
                    }
                }
        ...
        protected static ODataMessageReader CreateODataMessageReader(IODataResponseMessage responseMessage, ResponseInfo responseInfo, ref ODataPayloadKind payloadKind)
        {
            ...
            if (payloadKind == ODataPayloadKind.Unsupported)
            {
                ...
                {
                    throw DSClient.Error.InvalidOperation(Error.Format(SRResources.Materializer_InvalidContentTypeEncountered, responseMessage.GetHeader(XmlConstants.HttpContentType)));
                }
