        ...
        internal void SetRequestVersion(Version requestVersion, Version maxProtocolVersion)
        {
            if (requestVersion != null)
            {
                ...
                if (requestVersion.Major > 0)
                {
                    ...
                    {
                        this.SetHeader(XmlConstants.HttpODataVersion, requestVersion + Util.VersionSuffix);
                    }
                }
            }

            // for legacy reasons, we don't set this until we set a DSV
            this.SetHeader(XmlConstants.HttpODataMaxVersion, maxProtocolVersion + Util.VersionSuffix);
        }
