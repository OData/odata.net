...
namespace Microsoft.OData.Edm.Csdl
{
    ...
    public class CsdlReader
    {
        ...
        {
            return TryParse(reader, references, out model, out errors, true /*enableVocabularySupport*/);
        }
        ...
        /// <returns>Success of the parse operation.</returns>
        public static bool TryParse(XmlReader reader, IEnumerable<IEdmModel> references, out IEdmModel model, out IEnumerable<EdmError> errors, bool enableVocabularySupport)
        {
            EdmUtil.CheckArgumentNull(references, "references");
            CsdlReader edmxReader = new CsdlReader(reader, null);
            return edmxReader.TryParse(references, out model, out errors, enableVocabularySupport);
        }
        ...
        private bool TryParse(IEnumerable<IEdmModel> referencedModels, out IEdmModel model, out IEnumerable<EdmError> parsingErrors, bool enableVocabularySupport = true)
        {
            Version edmxVersion;
            CsdlModel astModel;
            ...
            if (!this.HasIntolerableError())
            {
                ...
                if (!this.HasIntolerableError())
                {
                    CsdlSemanticsModel tmp = new CsdlSemanticsModel(astModel, new CsdlSemanticsDirectValueAnnotationsManager(), referencedAstModels, enableVocabularySupport);

                    if (enableVocabularySupport)
                    {
                        tmp.AddToReferencedModels(VocabularyModelProvider.VocabularyModels);
                    }
