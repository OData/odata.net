    ...
    internal sealed class ReorderingJsonReader : BufferingJsonReader
    {
        ...
        public override Task<bool> CanStreamAsync()
        {
            ...
            if (getValueTask.IsCompletedSuccessfully)
            {
                ...
                return (value is string || value == null) ? CachedTasks.True : CachedTasks.False;
            }
        ...
        protected override async Task ProcessObjectValueAsync()
        {
            ...
            while (true)
            {
                ...
                {
                    case JsonNodeType.StartObject:
                        await this.HandleStartObjectAsync(bufferedObjectStack)
                            .ConfigureAwait(false);

                        break;
                    case JsonNodeType.EndObject:
                        if (await this.HandleEndObjectAsync(bufferedObjectStack)
                            .ConfigureAwait(false))
                        {
                        ...
                        break;
                    case JsonNodeType.Property:
                        await this.HandlePropertyAsync(bufferedObjectStack)
                            .ConfigureAwait(false);
        ...
        private ValueTask HandleStartObjectAsync(Stack<BufferedObject> bufferedObjectStack)
        {
            ...
            // See if it's an in-stream error
            Task processObjectValueTask = base.ProcessObjectValueAsync();
            if (!processObjectValueTask.IsCompletedSuccessfully)
            {
                return AwaitProcessObjectValueAsync(
                    this,
                    bufferedObject,
                    processObjectValueTask);
            }
            ...
            static async ValueTask AwaitProcessObjectValueAsync(
        ...
        private ValueTask HandlePropertyAsync(Stack<BufferedObject> bufferedObjectStack)
        {
            ...
            if (readPropertyNameTask.IsCompletedSuccessfully)
            {
                ...
                return ContinueAfterReadPropertyName(
                    this,
                    bufferedObject,
                    bufferedProperty,
                    propertyName,
            ...
            static ValueTask ContinueAfterReadPropertyName(
                ReorderingJsonReader thisParam,
                ...
                ValueTask<(string PropertyName, string AnnotationName)> pendingReadPropertyNameTask)
            {
                ...
                await ContinueAfterReadPropertyName(
                    thisParam,
                    bufferedObjectParam,
