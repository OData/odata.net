...
namespace Microsoft.OData.Json
{
    ...
    internal static class JsonReaderExtensions
    {
        ...
        internal static Task<string> ReadPropertyNameAsync(this IJsonReader jsonReader)
        {
            ...
            static async Task<string> AwaitGetPropertyNameAsync(IJsonReader localJsonReader, Task<string> pendingGetPropertyNameTask)
            {
                string propertyName = await pendingGetPropertyNameTask.ConfigureAwait(false);
                await localJsonReader.ReadNextAsync()
        ...
        internal static Task<object> ReadPrimitiveValueAsync(this IJsonReader jsonReader)
        {
            ...
            static async Task<object> AwaitGetValueAsync(IJsonReader localJsonReader, Task<object> pendingGetValueTask)
            {
                object value = await pendingGetValueTask.ConfigureAwait(false);
                await ReadNextAsync(localJsonReader, JsonNodeType.PrimitiveValue)
                    .ConfigureAwait(false);
        ...
        internal static Task SkipValueAsync(this IJsonReader jsonReader)
        {
            ...
            static async Task AwaitReadAsync(IJsonReader localJsonReader, int depth, Task<bool> pendingReadTask)
            {
                while (true)
                {
                    ...
                    // Process next node
                    depth = AdjustDepth(depth, localJsonReader.NodeType);

                    pendingReadTask = localJsonReader.ReadAsync();
                }
        ...
        internal static Task<ODataValue> ReadAsUntypedOrNullValueAsync(this IJsonReader jsonReader)
        {
            ...
            static async Task<ODataValue> AwaitSkipValueAsync(Task pendingSkipValueTask, StringBuilder localBuilder)
            {
                await pendingSkipValueTask.ConfigureAwait(false);
                Debug.Assert(localBuilder.Length > 0, "localBuilder.Length > 0");
                return new ODataUntypedValue()
                {
                    RawValue = localBuilder.ToString(),
                };
            }
        }
        ...
        internal static Task<JsonNodeType> ReadNextAsync(this IJsonReader jsonReader)
        {
            ...
            static async Task<JsonNodeType> AwaitReadAsync(Task<bool> pendingReadTask, IJsonReader localJsonReader)
            {
...
#endif
                return localJsonReader.NodeType;
            }
        }
