...
namespace Microsoft.OData.JsonLight
{
    ...
    internal sealed class ODataJsonLightCollectionReader : ODataCollectionReaderCoreAsync
    {
        ...
        protected override async Task<bool> ReadAtStartImplementationAsync()
        {
            ...
            // Position the reader on the first node depending on whether we are reading a nested payload or not
            await this.jsonLightCollectionDeserializer.ReadPayloadStartAsync(
                ODataPayloadKind.Collection,
                propertyAndAnnotationCollector,
                this.IsReadingNestedPayload,
        ...
        private async Task<bool> ReadAtCollectionStartImplementationAsynchronously()
        {
            ...
            {
                object item = await this.jsonLightCollectionDeserializer.ReadCollectionItemAsync(
                    this.ExpectedItemTypeReference,
        ...
        private async Task<bool> ReadAtCollectionEndImplementationAsynchronously()
        {
            ...
            await this.jsonLightCollectionDeserializer.ReadCollectionEndAsync(this.IsReadingNestedPayload)
                .ConfigureAwait(false);
            await this.jsonLightCollectionDeserializer.ReadPayloadEndAsync(this.IsReadingNestedPayload)
                .ConfigureAwait(false);
