...
namespace Microsoft.OData.UriParser
{
    ...
    internal static class TypePromotionUtils
    {
        ...
        /// and used to build the node's collection type. This enables early rejection of incompatible element scenarios (e.g. string items when a numeric collection is required).
        /// NOTE:
        /// This transformation is intentionally performed late (during promotion) because at this stage we have the target collection element type available to guide parsing.
        ...
        public static bool TryParseCollectionConstantNode(ConstantNode constantNode, IEdmCollectionTypeReference targetType, out CollectionConstantNode collectionConstantNode)
        {
            ...
            try
            {
                ...
                if (result != null && result is ODataCollectionValue collectionValue)
                {
                    ...
                    object collectionItem = collectionValue.Items?.FirstOrDefault(d => d != null);
                    if (collectionItem == null) // Empty collection or all items are null - can promote to any collection type
                    {
