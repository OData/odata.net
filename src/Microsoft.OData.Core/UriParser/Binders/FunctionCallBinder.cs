...
namespace Microsoft.OData.UriParser
{
    ...
    internal sealed class FunctionCallBinder : BinderBase
    {
        ...
        private static void TypePromoteArguments(
            ...
            (QueryNode Node, IEdmTypeReference TypeReference)[] argumentsMetadata)
        {
            ...
            for (int i = 0; i < argumentsMetadata.Length; i++)
            {
                ...
                QueryNode queryNode = argumentsMetadata[i].Node;
                ...
                if (argumentsMetadata[i].Node is SingleValueNode singleValueNode)
                {
                    if (signatureArgumentType.IsCollection() && singleValueNode is ConstantNode constantNode && constantNode.TypeReference.IsString())
                    {
                        ...
                        argumentsMetadata[i].Node = MetadataBindingUtils.ConvertToTypeIfNeeded(collectionConstantNode, signatureArgumentType);
                        continue;
                    }
                    ...
                    argumentsMetadata[i].Node = MetadataBindingUtils.ConvertToTypeIfNeeded(singleValueNode, signatureArgumentType);
                }
                else
                {
                    ...
                    CollectionNode argumentNode = (CollectionNode)argumentsMetadata[i].Node;
                    argumentsMetadata[i].Node = MetadataBindingUtils.ConvertToTypeIfNeeded(argumentNode, signatureArgumentType);
                }
            }
        }

        /// <summary>
        /// Checks that all arguments are single- or collection-valued nodes.
        /// </summary>
        ...
        private static (QueryNode Node, IEdmTypeReference TypeReference)[] ValidateArgumentNodes(string functionName, List<QueryNode> argumentNodes)
        {
            ...
            // Functions take a single- or collection-valued arguments
        ...
        private static KeyValuePair<string, FunctionSignatureWithReturnType> MatchSignatureToUriFunction(
            ...
            foreach (FunctionParameterToken paraToken in parameterTokens)
            {
                ...
                if (valueToken != null && (valueStr = valueToken.Value as string) != null && !string.IsNullOrEmpty(valueToken.OriginalText))
                {
                    ...
                    if (lexer.CurrentToken.Kind == ExpressionTokenKind.BracketedExpression || lexer.CurrentToken.Kind == ExpressionTokenKind.BracedExpression)
                    {
                        ...
                        LiteralToken newValueToken = new LiteralToken(result, valueToken.OriginalText, functionParameter?.Type);
