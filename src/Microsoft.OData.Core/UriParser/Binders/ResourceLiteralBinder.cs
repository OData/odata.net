    ...
    internal sealed class ResourceLiteralBinder
    {
        ...
        public static QueryNode BindResourceLiteralToken(ResourceLiteralToken resourceLiteralToken, Func<QueryToken, QueryNode> bindMethod, BindingState bindingState)
        {
            ...
            else
            {
                ...
                {
                    throw new ODataException(Error.Format(SRResources.ResourceLiteralBinder_IncompatibleType, typeFromLiteral.FullTypeName(), edmStructuredType.FullTypeName()));
                }
        ...
        private static ResourceConstantNode BindResourceLiteral(ResourceLiteralToken resourceLiteralToken, IEdmStructuredType structuredType, Func<QueryToken, QueryNode> bindMethod, BindingState bindingState)
        {
            ...
            IEdmStructuredTypeReference structuredTypeReference = structuredType == null ? null : structuredType.ToTypeReference(true) as IEdmStructuredTypeReference;
            ResourceConstantNode singleResourceNode = new ResourceConstantNode(structuredTypeReference)
            {
                LiteralText = resourceLiteralToken.OriginalText,
            };

            foreach (var item in resourceLiteralToken.Properties)
            {
                ...
                {
                    literalToken.ExpectedType = isNullOrUntyped ? null : propertyType;
                    QueryNode node = bindMethod(literalToken);
                    singleResourceNode.Properties.Add(new KeyValuePair<string, QueryNode>(item.Key, node));
                }
                else if (item.Value is ResourceLiteralToken subResourceLiteralToken)
                {
                    ...
                    BindingHelpers.SetExpectedType(subResourceLiteralToken, propertyType);

                    QueryNode resourceConstant = bindMethod(subResourceLiteralToken);
                    singleResourceNode.Properties.Add(new KeyValuePair<string, QueryNode>(item.Key, resourceConstant));
                }
                else if (item.Value is CollectionLiteralToken subCollectionToken)
                {
                    ...
                    BindingHelpers.SetExpectedType(subCollectionToken, propertyType);

                    QueryNode collectionNode = bindMethod(subCollectionToken);
                    singleResourceNode.Properties.Add(new KeyValuePair<string, QueryNode>(item.Key, collectionNode));
                }
                else
                {
                    throw new ODataException(Error.Format(SRResources.ResourceLiteralBinder_InvalidPropertyValueTokenItem, item.Value.Kind));
                }
            }
        ...
        private static IEdmStructuredType ResolveActualType(ResourceLiteralToken resourceLiteralToken, BindingState bindingState)
        {
            ...
            string typeNameFromLiteral = resourceLiteralToken.TypeNameFromLiteral;
            if (typeNameFromLiteral.StartsWith('#'))
            {
                typeNameFromLiteral = typeNameFromLiteral.Substring(1);// remove #
            }
            else if (typeNameFromLiteral.StartsWith("%23")) // In case the # is escaped in the URL.
            {
                typeNameFromLiteral = typeNameFromLiteral.Substring(3);// remove %23
            }

            IEdmModel model = bindingState.Model;

            IEdmStructuredType actualType = bindingState.Configuration.Resolver.ResolveType(model, typeNameFromLiteral) as IEdmStructuredType;
            if (actualType == null)
            {
                // Beside throwing exception, One option is to create a 'EdmUntypedStructuredType' using the '@odata.type' value.
                // But we don't want to do that because it may cause confusion.
                // For example, if the '@odata.type' value is 'Namespace.Customer', it's more likely that the user is trying to specify a type name rather than trying to create an untyped structured type with the name 'Namespace.Customer'.
                throw new ODataException(Error.Format(SRResources.ResourceLiteralBinder_CannotResolveTypeName, resourceLiteralToken.TypeNameFromLiteral, resourceLiteralToken.OriginalText.ToString()));
            }
