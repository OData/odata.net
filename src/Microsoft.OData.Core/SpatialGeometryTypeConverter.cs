...
namespace Microsoft.OData
{
    ...
    internal sealed class SpatialGeometryTypeConverter : IPrimitiveTypeConverter
    {
        public void WriteJson(object instance, IJsonWriter jsonWriter)
        {
            Debug.Assert(jsonWriter != null);
            Debug.Assert(instance != null);
            Debug.Assert(instance is Geometry, $"Expected instance of type {nameof(Geometry)} but got {instance.GetType()}.");

            WriteGeometry(jsonWriter, instance as Geometry, writeCrs: true);
        }

        public async Task WriteJsonAsync(object instance, IJsonWriter jsonWriter)
        {
            Debug.Assert(jsonWriter != null);
            Debug.Assert(instance != null);
            Debug.Assert(instance is Geometry, $"Expected instance of type {nameof(Geometry)} but got {instance.GetType()}.");

            await WriteGeometryAsync(jsonWriter, instance as Geometry, writeCrs: true).ConfigureAwait(false);
        }

        private static void WriteGeometry(IJsonWriter jsonWriter, Geometry geometry, bool writeCrs)
        {
            ...
            switch (geometry)
            {
                ...
                default:
                    throw new ODataException(
                        Error.Format(SRResources.SpatialPrimitiveTypeConverter_UnsupportedGeometryType, typeof(SpatialGeometryTypeConverter).FullName, geometry.GeometryType));
            }
        ...
        private static void WriteGeometryCollection(IJsonWriter jsonWriter, GeometryCollection collection)
        {
            jsonWriter.StartArrayScope();
        ...
        private static void WriteCrs(IJsonWriter jsonWriter, int srid)
        {
        ...
        private static async Task WriteGeometryAsync(IJsonWriter jsonWriter, Geometry geometry, bool writeCrs)
        {
        ...
        private static async Task WriteGeometryCollectionAsync(IJsonWriter jsonWriter, GeometryCollection collection)
        {
            await jsonWriter.StartArrayScopeAsync().ConfigureAwait(false);
        ...
        private static async Task WriteCrsAsync(IJsonWriter jsonWriter, int srid)
        {
