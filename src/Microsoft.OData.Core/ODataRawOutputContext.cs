...
namespace Microsoft.OData
{
    ...
    internal class ODataRawOutputContext : ODataOutputContext
    {
        ...
        /// <remarks>The write must flush the output when it's finished (inside the last Write call).</remarks>
        internal async override ValueTask<ODataAsynchronousWriter> CreateODataAsynchronousWriterAsync()
        {
            this.AssertAsynchronous();

            ODataAsynchronousWriter writer = await TaskUtils.GetTaskForSynchronousOperation(() => this.CreateODataAsynchronousWriterImplementation());
            return writer;
        }
