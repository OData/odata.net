//---------------------------------------------------------------------
// <copyright file="ODataAtomMetadataDeserializer.cs" company="Microsoft">
...
namespace Microsoft.OData.Core.Atom
{
    ...
    internal abstract class ODataAtomMetadataDeserializer : ODataAtomDeserializer
    {
        ...
        protected string ReadElementStringValue()
        {
            ...
            // BUG: Server uses special algorithm to read element content; use the server behavior for
            //             ODL and server scenarios; client scenario continues to use custom client algorithm (first text node; see above)
            // TODO: Client reads the m:type attribute even on the EPM elements. It seems it doesn't actually use the result anywhere though
            // so we might be able to not do that at all.
