...
Unlike metadata or service document, you must create another writer from `ODataMessageWriter` to write an entity set. The library is designed to write entity set in a streaming/progressive way, which means the entity is written one by one.
