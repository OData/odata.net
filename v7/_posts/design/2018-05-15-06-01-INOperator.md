...
A static collection is a defined as a comma-delimited list of objects of the same type and is enclosed by parentheses or brackets (note: see Exclusions of Implementation regarding use of brackets). A collection expression is represented by a collection in the EDM model; the client can traverse navigation properties from the original entity collection to access other collections.
...
The ABNF indicates that brackets are supported for enclosing the lists; however, this particular feature is currently excluded from implementation due to complexity in code. For now, lists are supported only with parentheses.
...
`GET /service/People?$filter=Name in SiblingNames`

Result: Members of People whose Name is equal to any member of the string collection SiblingNames.

Note: In other words, this query is attempting to retrieve people who share the same names as your siblings.

#### Filtering members with IN operator using single value as constant
...
`GET /service/Shipments?$filter=ShipToAddress/CountryCode in MyShippers/Regions`

Result: Members of Shipments whose CountryCode string property in the ShipToAddress navigation property is equal to any member of the Regions string collection in the MyShippers navigation property.
...
We will need to create a binder to consume an InToken and create an InNode. See `src\Microsoft.OData.Core\UriParser\Binders\MetadataBinder.cs` for use cases on the MetadataBinder. Note that the new binder does not derive from the MetadataBinder class; see `src\Microsoft.OData.Core\UriParser\Binders\BinaryOperatorBinder.cs` as sample implementation.
