...
#pragma warning restore 0169, 0649
            ...
            IDataService dataService)
        {
            ...
            {
                Version unused;
                description.VerifyAndRaiseResponseVersion(dataService.Provider.GetMetadataResponseAndEdmSchemaVersions(out unused), dataService);
            }
            ...
            if (description.TargetSource != RequestTargetSource.ServiceOperation)
            {
                if (requestMessage.HttpVerb.IsInsert())
                {
                    ...
                    {
                        Debug.Assert(description.ResponseVersion == dataService.OperationContext.RequestMessage.RequestMinVersion || newDescription.Preference.HasResponseBodyPreference, "description.ResponseVersion == dataService.OperationContext.RequestMessage.RequestMinVersion || newDescription.Preference != Preference.None");
                ...
                else if (requestMessage.HttpVerb.IsUpdate())
                {
                    ...
                    {
                        Debug.Assert(
                            description.ResponseVersion == dataService.OperationContext.RequestMessage.RequestMinVersion || newDescription.Preference.HasResponseBodyPreference,
                            "If no Prefer header was applied for a 204 response then the response version must be the MinDSV specified in the request.");
                        statusCode = 204; // 204 - No Content
                    }
                }
...
#if DEBUG
                    Debug.Assert(description.ResponseVersion == dataService.OperationContext.RequestMessage.RequestMinVersion, "description.ResponseVersion == dataService.OperationContext.RequestMessage.RequestMinVersion");
#endif
        ...
        private static void HandleDeleteOperation(RequestDescription description, IDataService dataService)
        {
            ...
            else if (description.IsSingleResult && description.TargetKind == RequestTargetKind.Resource)
            {
                ...
                // TODO: SQLBUDT 672375: move this check to start of the method once this bug is approved
                ...
                // TODO: SQLBUDT 602945 we need to fix this: with the new etag design, we will have type information in the etag.
            ...
            IODataResponseMessage responseMessage)
        {
            ...
            try
            {
                ...
                else if (requestMessage.HttpVerb == HttpVerbs.PUT || requestMessage.HttpVerb == HttpVerbs.MERGE || requestMessage.HttpVerb == HttpVerbs.PATCH)
                {
                    ResourceSetWrapper container;
                    ...
                    {
                        Debug.Assert(
                            description.ResponseVersion == dataService.OperationContext.RequestMessage.RequestMinVersion || description.Preference.HasResponseBodyPreference,
                            "For update requests, which does not need payload to be generated, the response version must be the same as the min DSV or preference must be applied.");
                ...
                else
                {
                    ...
                    {
                        Debug.Assert(
                            description.Preference.ShouldNotIncludeResponseBody && description.ResponseVersion >= VersionUtil.Version3Dot0,
                            "The preference no-content should be applied which means the response version must be at least 3.0");
                        Debug.Assert(dataService.OperationContext.ResponseMessage.StatusCode == 204, "Ensuring that status code is 204");

                        // Write the DataServiceId header from the entity's id.
                        dataService.OperationContext.ResponseMessage.SetHeader(XmlConstants.HttpDataServiceId, entityToSerialize.SerializedKey.Identity);
                        WebUtil.WriteETagValueInResponseHeader(description, etagValue, dataService.OperationContext.ResponseMessage);
                        return WebUtil.GetEmptyStreamWriter();
                    }
                }
        ...
        private Action<Stream> HandleRequest()
        {
            ...
            catch (Exception ex)
            {
                ...
                // Hence, return DSV 1.0.
                this.operationContext.ResponseMessage.SetHeader(XmlConstants.HttpDataServiceVersion, XmlConstants.DataServiceVersion1Dot0 + ";");
                throw;
            }
        ...
        private Action<Stream> HandleBatchRequest()
        {
            ...
            catch (ODataException odataException)
            {
                ...
                throw DataServiceException.CreateBadRequestError(System.Data.Services.Strings.DataServiceException_GeneralError, odataException);
            }
        ...
        private class BatchDataService : IDataService
        {
            ...
            internal void HandleBatchContent(Stream responseStream)
            {
                ...
                try
                {
                    while (!this.batchLimitExceeded && this.batchReader.State != ODataBatchReaderState.Completed)
                    {
                        ...
                        IODataRequestMessage operationRequestMessage;
                        ...
                        catch (Exception exception)
                        {
                            ...
                            {
                                exception = DataServiceException.CreateBadRequestError(System.Data.Services.Strings.DataServiceException_GeneralError, exception);
                            }
                            ...
                            ErrorHandler.HandleBatchOperationError(this, null, null, exception, batchWriter, responseStream, this.dataService.OperationContext.RequestMessage.RequestMinVersion);
                            break;
                        }
                        ...
                        try
                        {
                            switch (this.batchReader.State)
                            {
                                ...
                                case ODataBatchReaderState.Operation:
                                    this.HandleBatchOperation(operationRequestMessage, batchWriter, exceptionEncounteredInChangeset != null, ref serviceOperationRequests);
                                    break;
                                default:
                        ...
                        catch (Exception exception)
                        {
                            ...
                            if (this.batchReader.State == ODataBatchReaderState.ChangesetEnd)
                            {
                                ...
                                this.HandleChangesetException(exception, this.batchOperationContexts, batchWriter, responseStream, this.dataService.OperationContext.RequestMessage.RequestMinVersion);
                                changesetStarted = false;
                            }
                            ...
                            else
                            {
                                ...
                                try
                                {
                                    ...
                                    ErrorHandler.HandleBatchOperationError(this, requestMessage, responseMessage, exception, batchWriter, responseStream, this.dataService.OperationContext.RequestMessage.RequestMinVersion);
                                }
                                finally
                                {
            ...
            private static BatchServiceHost CreateBatchServiceHostFromOperationMessage(IDataService dataService, IODataRequestMessage operationRequestMessage, HashSet<int> contentIds, ODataBatchWriter writer)
            {
                ...
                Version maxDataServiceVersion = dataService.OperationContext.RequestMessage.RequestMaxVersion;
                Version minDataServiceVersion = dataService.OperationContext.RequestMessage.RequestMinVersion;
                Version dataServiceVersion = dataService.OperationContext.RequestMessage.RequestVersion;

                // If the Content-ID header is defined, it should be unique.
                string contentIdValue = operationRequestMessage.GetHeader(XmlConstants.HttpContentID);
                ...
                return new BatchServiceHost(absoluteServiceUri, operationRequestMessage, contentIdValue, writer, maxDataServiceVersion, minDataServiceVersion, dataServiceVersion);
            }
            ...
            private void HandleBatchOperation(IODataRequestMessage operationRequestMessage, ODataBatchWriter batchWriter, bool ignoreCUDOperations, ref bool serviceOperationRequests)
            {
