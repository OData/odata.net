...
namespace System.Data.Services
{
    ...
    internal class RequestDescription
    {
...
#if DEBUG
            this.MinimumRequestVersion = VersionUtil.Version1Dot0;
            this.maxFeatureVersion = VersionUtil.Version1Dot0;
#endif
        ...
        internal Version MinimumRequestVersion { get; private set; }
        ...
        internal bool IsETagHeaderAllowed
        {
            get
            {
                ...
                return this.IsSingleResult && this.CountOption != RequestQueryCountOption.ValueOnly && (this.RootProjectionNode == null || !this.RootProjectionNode.ExpansionsSpecified) && !this.LinkUri;
            }
        }
        ...
        internal void AnalyzeClientPreference(IDataService service)
        {
            ...
            Debug.Assert(
                (this.RequestVersion < VersionUtil.Version3Dot0) || (this.serviceMaxProtocolVersion >= VersionUtil.Version3Dot0) /*logical implication*/,
        ...
        {
            RequestQueryCountOption countOption = this.LastSegmentInfo.Identifier == XmlConstants.UriCountSegment ? RequestQueryCountOption.ValueOnly : RequestQueryCountOption.None;
            ...
            switch (this.TargetKind)
            {
                ...
                default:
                    ...
                    if (payloadKind != ODataPayloadKind.Unsupported)
                    {
                        ...
                        Debug.Assert(version <= ODataVersion.V3, "Unexpected version, is there a new one?");
        ...
        internal void UpdateVersion(ResourceSetWrapper resourceSet, IDataService service)
        {
                ...
                (host.HttpVerb == HttpVerbs.POST && this.TargetSource == RequestTargetSource.ServiceOperation))
            {
                ...
                if (this.TargetKind == RequestTargetKind.Resource)
                {
                    ...
                    if (!this.LinkUri)
                    {
                        ...
                        // EPM must only bump the response version for Atom responses
                        bool considerEpmInVersion = this.IsAtomResponse;
                    ...
                    this.CountOption != RequestQueryCountOption.ValueOnly &&
            ...
            {
                // For patch, we should ensure that the MPV in the server is set to 3.0 and the request DSV is 3.0.
                if (service.Configuration.DataServiceBehavior.MaxProtocolVersion < DataServiceProtocolVersion.V3)
                {
                    // incorrect versioning for PATCH verb should cause a 405 - MethodNotAllowed error
                    throw DataServiceException.CreateMethodNotAllowed(
                                Strings.RequestUriProcessor_MethodNotAllowed,
                                DataServiceConfiguration.GetAllowedMethods(service.Configuration, resourceSet, this));
                }

                this.VerifyRequestVersion(VersionUtil.Version3Dot0, service);
            }
        }
        ...
        internal void UpdateResponseVersionForPostMediaResource(ResourceType resourceType, IDataService dataService)
        {
            ...
            if (!this.Preference.ShouldNotIncludeResponseBody)
            {
                ...
                bool isAtomResponse = this.IsAtomResponse;

                ResourceSetWrapper resourceSet = this.LastSegmentInfo.TargetResourceSet;
                Version minimumPayloadVersion = resourceType.GetMinimumResponseVersion(dataService, resourceSet, isAtomResponse);
                if (isAtomResponse && minimumPayloadVersion <= VersionUtil.Version2Dot0)
                {
                    // Update the actual response version to the minimumPayloadVersion
                    this.VerifyAndRaiseActualResponseVersion(minimumPayloadVersion, dataService);
                    minimumPayloadVersion = resourceType.GetMinimumResponseVersion(dataService, resourceSet, false);
                }
            ...
            {
                Debug.Assert(
                    this.ResponseVersion >= VersionUtil.Version3Dot0,
                    "If the return-no-content Prefer header is applied, the response version must be at least 3.0.");
            }
        }
        ...
        internal void VerifyRequestVersion(Version requiredVersion, IDataService service)
        {
            ...
            this.MinimumRequestVersion = VersionUtil.RaiseVersion(this.MinimumRequestVersion, requiredVersion);
#endif
        ...
        internal void VerifyAndRaiseResponseVersion(Version version, IDataService service)
        {
            ...
            // The requestMinVersion has already been set to the MinDSV received in the request. So, 
            // the ResponseVersion can never be less than MinDSV. So, do the validation only against 
            // requestMaxVersion
        ...
        {
            this.VerifyAndRaiseResponseVersion(service.OperationContext.RequestMessage.RequestMinVersion, service);
        }
        ...
        {
            // All the 3 versions must always remain in valid state
            // 1> MPV in the server must always be greater than or equal to the minimum feature version.
            // 2> Request DSV header values must always be greater or equal to the minimum request version.
            // 3> Request MaxDSV header value must always be greater than or equal to the response version.            
            ...
            Debug.Assert(this.MinimumRequestVersion <= this.RequestVersion, "this.requireMinimumVersion <= Request DSV");
            ...
            // It is not possible to verify the consistency between ResponseVersion and service.OperationContext.RequestMessage.RequestMinVersion, since we do not know when 
            // the ResponseVersion will be bumped-up to the requestMinVersion.
        }
...
#if DEBUG
            this.MinimumRequestVersion = other.MinimumRequestVersion;
