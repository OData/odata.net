...
namespace System.Data.Services.Serializers
{
    ...
    internal sealed class EntityDeserializer : ODataMessageReaderDeserializer
    {
        ...
        private void ApplyEntityProperties(SegmentInfo segmentInfo, ODataEntry entry, ODataEntryAnnotation entryAnnotation)
        {
            ...
            // Note that in ATOM case which will also apply all EPM values, since ODataLib includes those in the properties
            // on the entry and hides the EPM from us.
        ...
        private ResourceType GetEntryResourceType(ODataEntry entry, ResourceType expectedType)
        {
            ...
            else
            {
                ...
                {
                    throw DataServiceException.CreateBadRequestError(System.Data.Services.Strings.BadRequest_InvalidTypeName(payloadTypeName));
                }

                if (!expectedType.IsAssignableFrom(resourceType))
                {
                    throw DataServiceException.CreateBadRequestError(System.Data.Services.Strings.BadRequest_InvalidTypeSpecified(payloadTypeName, expectedType.FullName));
                }
            }
