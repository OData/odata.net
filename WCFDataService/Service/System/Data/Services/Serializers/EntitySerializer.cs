...
namespace System.Data.Services.Serializers
{
    ...
    internal sealed class EntitySerializer : Serializer
    {
            ...
            get
            {
                if (this.operationSerializer == null)
                {
                    ...
                    this.operationSerializer = new OperationSerializer(
                        ...
                        this.Service.Provider.ContainerName, 
                        this.contentFormat,
        ...
        {
            Uri associationLinkUri = RequestUriProcessor.AppendEscapedSegment(entityToSerialize.SerializedKey.RelativeEditLink, XmlConstants.UriLinkSegment);
            return RequestUriProcessor.AppendUnescapedSegment(associationLinkUri, navigationProperty.Name);
        }
        ...
        /// </returns>
        private ODataAssociationLink GetAssociationLink(EntityToSerialize entityToSerialize, ResourceProperty navigationProperty)
        {
            Debug.Assert(entityToSerialize != null, "entityToSerialize != null");
            Debug.Assert(navigationProperty.TypeKind == ResourceTypeKind.EntityType, "navigationProperty.TypeKind == ResourceTypeKind.EntityType");

            ODataAssociationLink link = new ODataAssociationLink
            {
                Name = navigationProperty.Name,
            };

            this.PayloadMetadataPropertyManager.SetUrl(link, () => GetAssociationLinkUrl(entityToSerialize, navigationProperty));
        ...
        private void WriteEntry(IExpandedResult expanded, object element, bool resourceInstanceInFeed, ResourceType expectedType)
        {
            ...
            {
                entry.SetSerializationInfo(new ODataFeedAndEntrySerializationInfo { EntitySetName = this.CurrentContainer.Name, EntitySetElementTypeName = this.CurrentContainer.ResourceType.FullName, ExpectedTypeName = expectedType.FullName });
            }
            ...
            if (actualResourceType.ResourceTypeKind != ResourceTypeKind.EntityType)
            {
                ...
                throw new DataServiceException(500, Services.Strings.BadProvider_InconsistentEntityOrComplexTypeUsage(actualResourceType.FullName));
            }
            ...
            // Populate association links (has to be done before the WriteStart since JSON writes these at the beginning)
            entry.AssociationLinks = this.GetEntityAssociationLinks(entityToSerialize, projectionNodes);
            ...
            bool topLevel)
        {
            ...
            feed.SetSerializationInfo(new ODataFeedAndEntrySerializationInfo { EntitySetName = this.CurrentContainer.Name, EntitySetElementTypeName = this.CurrentContainer.ResourceType.FullName, ExpectedTypeName = expectedType.FullName });

            // Write the other elements for the feed
            this.PayloadMetadataPropertyManager.SetId(feed, () => getAbsoluteUri().AbsoluteUri);
            ...
            // support for $inlinecount
            bool writeInlineCountAtEnd = false;
            if (topLevel && this.RequestDescription.CountOption == RequestQueryCountOption.Inline)
            {
                writeInlineCountAtEnd = this.contentFormat == ODataFormat.VerboseJson;
                if (!writeInlineCountAtEnd)
                {
        ...
        private IEnumerable<ODataAssociationLink> GetEntityAssociationLinks(EntityToSerialize entityToSerialize, IEnumerable<ProjectionNode> projectionNodesForCurrentResourceType)
        {
            Debug.Assert(entityToSerialize != null, "entityToSerialize != null");
            Debug.Assert(
                projectionNodesForCurrentResourceType == null ||
            ...
            if (!this.Service.Configuration.DataServiceBehavior.ShouldIncludeAssociationLinksInResponse)
            {
                // ODataLib treats null enumerations as empty enumerations, so it's faster this way instead of returning an empty enumeration.
                return null;
            }

            List<ODataAssociationLink> associationLinks = new List<ODataAssociationLink>(entityToSerialize.ResourceType.Properties.Count);
            if (projectionNodesForCurrentResourceType == null)
            {
                foreach (ResourceProperty property in this.Provider.GetResourceSerializableProperties(this.CurrentContainer, entityToSerialize.ResourceType))
                {
                    ...
                    {
                        associationLinks.Add(this.GetAssociationLink(entityToSerialize, property));
                    }
                }
            }
            else
            {
                ...
                {
                    ResourceProperty property = projectionNode.Property;
                    if (property != null)
                    {
                        if (property.TypeKind == ResourceTypeKind.EntityType)
                        {
        ...
        private IEnumerable<ODataProperty> GetAllEntityProperties(EntityToSerialize entityToSerialize)
        {
            ...
            if (entityToSerialize.ResourceType.IsOpenType)
            {
                foreach (KeyValuePair<string, object> property in this.Provider.GetOpenPropertyValues(entityToSerialize.Entity))
                {
                    ...
                    {
                        throw new DataServiceException(500, Services.Strings.Syndication_InvalidOpenPropertyName(entityToSerialize.ResourceType.FullName));
                    }

                    properties.Add(this.GetODataPropertyForOpenProperty(propertyName, property.Value));
                }

                // Make sure all EntityPropertyMappings have values, and give them a value of null if they are empty (In case of Atom)
                // It is possible for GetOpenPropertyValues to skip (i.e. return empty) EPM properties
                if (entityToSerialize.ResourceType.HasEntityPropertyMappings && this.contentFormat == ODataFormat.Atom)
                {
                    var propertiesLookup = new HashSet<string>(properties.Select(p => p.Name));
                    foreach (EpmSourcePathSegment epmProperty in entityToSerialize.ResourceType.EpmSourceTree.Root.SubProperties
                                                                    .Where(p => !propertiesLookup.Contains(p.PropertyName)))
                    {
                        Debug.Assert(!entityToSerialize.ResourceType.Properties.Select(x => x.Name).Contains(epmProperty.PropertyName), "Empty EntityPropertyMapping type found that should have been populated.");
                        properties.Add(this.GetODataPropertyForOpenProperty(epmProperty.PropertyName, this.Provider.GetOpenPropertyValue(entityToSerialize.Entity, epmProperty.PropertyName)));
                    }
                }
            }
        ...
        private IEnumerable<ODataProperty> GetProjectedEntityProperties(EntityToSerialize entityToSerialize, IEnumerable<ProjectionNode> projectionNodesForCurrentResourceType)
        {
            ...
            // Write all the EPM properties In case it's relevant to current format. Since we need to write out all the EPM mappings also,
            // we need to pass all the values of the property that are getting mapped.
            // The code which determines which properties to write is temporary - Ideally, ODataLib should expose this functionality in some helper method.
            if (entityToSerialize.ResourceType.HasEntityPropertyMappings && this.contentFormat == ODataFormat.Atom)
            {
                foreach (EpmSourcePathSegment epmSegment in entityToSerialize.ResourceType.EpmSourceTree.Root.SubProperties)
                {
                    string propertyName = epmSegment.PropertyName;

                    // first check if the property is already present in the projectionNodes
                    if (properties.FirstOrDefault(p => p.Name == propertyName) == null)
                    {
                        // Note that we are adding entire complex value and just subproperties.
                        // We might implement an optimalization which would only fill the properties needed by EPM
                        // but for that we would need a more involved way of determining the EPM source tree from ODataLib
                        ResourceProperty property = entityToSerialize.ResourceType.TryResolvePropertyName(propertyName, exceptKind: ResourcePropertyKind.Stream);

                        object propertyValue = WebUtil.GetPropertyValue(this.Provider, entityToSerialize.Entity, entityToSerialize.ResourceType, property, property == null ? propertyName : null);
                        if (property != null)
                        {
                            properties.Add(new ODataProperty()
                            {
                                Name = propertyName,
                                Value = this.GetPropertyValue(propertyName, property.ResourceType, propertyValue, false /*openProperty*/)
                            });
                        }
                        else
                        {
        ...
        private ODataProperty GetODataPropertyForOpenProperty(string propertyName, object propertyValue)
        {
            ...
            else
            {
                ...
                {
                    throw new DataServiceException(500, Services.Strings.Syndication_InvalidOpenPropertyType(propertyName));
                }
            }
