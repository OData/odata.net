...
namespace System.Data.Services.Serializers
{
    ...
    internal abstract class Deserializer : IDisposable
    {
        ...
        /// </summary>
        protected abstract bool IsVerboseJsonRequest { get; }
        ...
        internal static Deserializer CreateDeserializer(RequestDescription description, IDataService dataService, bool update, UpdateTracker tracker)
        {
            ...
            Debug.Assert(
                ...
                (update /*PUT,MERGE,PATCH*/ && (dataService.OperationContext.RequestMessage.HttpVerb == HttpVerbs.PUT || 
                    dataService.OperationContext.RequestMessage.HttpVerb == HttpVerbs.MERGE ||
                    dataService.OperationContext.RequestMessage.HttpVerb == HttpVerbs.PATCH)),
                "For PUT, MERGE and PATCH, update must be true; for POST, update must be false");
        ...
        internal static void SetStreamPropertyValue(object resourceToBeModified, Stream requestStream, IDataService service, RequestDescription description)
        {
            ...
            if (service.OperationContext.RequestMessage.HttpVerb == HttpVerbs.MERGE)
            {
        ...
        internal static int GetIndexOfEntityResourceToModify(RequestDescription description)
        {
            ...
            if (description.LinkUri)
            {
                Debug.Assert(
                    ...
                    "There should be only one segment allowed after $link");
                ...
                Debug.Assert(
                    ...
                    "The segment previous to the $links segment must refer to a single resource");
        ...
        internal static Dictionary<string, object> ReadPayloadParameters(SegmentInfo actionSegment, IDataService dataService)
        {
            ...
            // Verify that the DSV is 3.0 or greater for parameters payload.
            requestDescription.VerifyRequestVersion(VersionUtil.Version3Dot0, dataService);
        ...
        protected void UpdateAndCheckRequestResponseDSV(ResourceType resourceType, bool topLevel)
        {
            ...
            // Note that we should do this regardless of the deserializer we're in, but we had a bug which is now a feature
            // where we only called this method from the SyndicationDeserializer and thus we should only do this in that case as well.
            // So for backward compat reasons we will not raise the version to 2.0 even if the response is ATOM but the request is JSON
            //   but we will raise it to 3.0 since that's not a breaking change anymore.
            if (this.IsAtomRequest)
            {
                Version minVersion = resourceType.EpmMinimumDataServiceProtocolVersion.ToVersion();
                this.RequestDescription.VerifyRequestVersion(minVersion, this.service);
            }
            ...
            if (topLevel && this.ResponseWillBeSent)
            {
                ...
                bool isAtomResponse = this.RequestDescription.IsAtomResponse;
                ...
                Version minimumPayloadVersion;
                if (!this.IsVerboseJsonRequest)
                {
                    // If the request was ATOM or JsonLight- then apply versioning as usual for any version
                    minimumPayloadVersion = resourceType.GetMinimumResponseVersion(this.Service, resourceSet, isAtomResponse);
                }
                else
                {
                    // If the request was JSON, we have the feature (was bug) that we didn't raise the response to 2.0 for EPM
                    // So first try to ask for version with EPM, if that returns 3.0 or higher, use that.
                    // Otherwise, ask for version without EPM (the old behavior) and use that instead.
                    // (only do this if EPM is applicable, that is it is an ATOM response)
                    minimumPayloadVersion = resourceType.GetMinimumResponseVersion(this.Service, resourceSet, isAtomResponse);
                    if (isAtomResponse && minimumPayloadVersion <= VersionUtil.Version2Dot0)
                    {
                        ...
                        this.RequestDescription.VerifyAndRaiseActualResponseVersion(minimumPayloadVersion, this.service);
                        minimumPayloadVersion = resourceType.GetMinimumResponseVersion(this.Service, resourceSet, false);
                    }
                }
