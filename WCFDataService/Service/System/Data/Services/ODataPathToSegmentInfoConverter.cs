...
namespace System.Data.Services
{
    ...
    internal sealed class ODataPathToSegmentInfoConverter
    {
        ...
        internal IList<SegmentInfo> ConvertPath(ODataPath path)
        {
            ...
            foreach (ODataPathSegment segment in path)
            {
                ...
                else
                {
                    ...
                    {
                        segmentInfo = CreateLinksSegment(previous);
        #if DEBUG
            ...
            if (segmentInfos.Count > 0 && segmentInfos[segmentInfos.Count - 1].TargetKind == RequestTargetKind.Link)
            {
        ...
        private static SegmentInfo CreateLinksSegment(SegmentInfo previous)
        {
        ...
        private static SegmentInfo CreateValueSegment(SegmentInfo previous)
        {
            ...
            if (previous.TargetKind == RequestTargetKind.Primitive)
            {
                ...
                {
                    throw DataServiceException.CreateBadRequestError(Strings.BadRequest_ValuesCannotBeReturnedForSpatialTypes);
                }
            }
        ...
        private SegmentInfo CreateFirstSegment(ODataPathSegment segment)
        {
            ...
            OperationSegment serviceOperation = segment as OperationSegment;
            if (serviceOperation != null)
            {
                Debug.Assert(serviceOperation.Operations.Count() == 1, "Operation segment should only ever have exactly one operation. Was a change made to how MetadataProviderEdmModel finds actions/service operations");
                var functionImport = serviceOperation.Operations.Single();
                var operation = ((MetadataProviderEdmFunctionImport)functionImport).ServiceOperation;
        ...
        private SegmentInfo CreateNextSegment(SegmentInfo previous, ODataPathSegment segment)
        {
            ...
            if (actionSegment != null)
            {
                ...
                serviceAction = ((MetadataProviderEdmFunctionImport)actionSegment.Operations.Single()).ServiceOperation;
        ...
        private bool TryCreateTypeNameSegment(SegmentInfo previous, ODataPathSegment segment, out SegmentInfo typeNameSegment)
        {
            ...
            if (!previous.SingleResult)
            {
                VersionUtil.CheckMaxProtocolVersion(VersionUtil.Version3Dot0, this.maxProtocolVersion);
            }
