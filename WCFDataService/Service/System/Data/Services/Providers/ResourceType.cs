    ...
    public class ResourceType
    {
        ...
        public bool IsOpenType
        {
            ...
            set
            {
                ...
                if (this.resourceTypeKind == ResourceTypeKind.ComplexType && value)
                {
        ...
        {
            get { return this.epmInfoInitialized; }
        }
        ...
        internal DataServiceProtocolVersion EpmMinimumDataServiceProtocolVersion
        {
            get
            {
                ...
                Debug.Assert(this.EpmInfoInitialized, "this.EpmInfoInitialized -- Epm Info must be initialized to determine version.");

                DataServiceProtocolVersion minDSPV;
                if (this.HasEntityPropertyMappings)
                {
                    minDSPV = this.EpmTargetTree.MinimumDataServiceProtocolVersion;
                }
                else
                {
                    minDSPV = DataServiceProtocolVersion.V1;
                }

                Debug.Assert(minDSPV <= DataServiceProtocolVersion.V3, "V3 is the maximum version supported at the moment");
                return minDSPV;
            }
        }
        ...
        internal EpmSourceTree EpmSourceTree
        {
            get
            {
                if (this.epmInfo == null)
                {
                    this.epmInfo = new EpmInfoPerResourceType(false /*isEFProvider*/);
                }
        ...
        {
            this.AddEntityPropertyMappingAttributeInternal(attribute, false /*isEFProvider*/);
        }
        ...
        internal void AddEntityPropertyMappingAttributeInternal(EntityPropertyMappingAttribute attribute, bool isEFProvider)
        {
            ...
            Debug.Assert(this.epmInfo.EFProvider == isEFProvider, "epmInfo.EFProvider == efProvider");
        ...
        internal Version GetMinimumResponseVersion(IDataService service, ResourceSetWrapper resourceSet, bool considerEpmInVersion)
        {
            ...
            Version minimumVersion = VersionUtil.Version1Dot0;

            // If target set contains collection properties we need v3.0
            // If target type contains named streams, we need v3.0
            minimumVersion = VersionUtil.RaiseVersion(minimumVersion, this.GetMinimumProtocolVersion(considerEpmInVersion));

            // If target type contains navigation property and ShouldIncludeAssociationLinksInResponse is on, we need v3.0
            // We only need to check this while computing the response version
            if (service.Configuration.DataServiceBehavior.ShouldIncludeAssociationLinksInResponse &&
                resourceSet.GetEntitySerializableProperties(service.Provider, this).Any(p => p.TypeKind == ResourceTypeKind.EntityType))
            {
                minimumVersion = VersionUtil.RaiseVersion(minimumVersion, VersionUtil.Version3Dot0);
            }
        ...
        internal Version GetMinimumProtocolVersion(bool considerEpmInVersion)
        {
            ...
            if (considerEpmInVersion)
            {
                ...
                Version epmMinVersion = this.EpmMinimumDataServiceProtocolVersion.ToVersion();
                minimumVersion = VersionUtil.RaiseVersion(minimumVersion, epmMinVersion);
            }
        ...
        private static bool EpmPropertyExistsInDeclaredProperties(EntityPropertyMappingAttribute epmAttr, ReadOnlyCollection<ResourceProperty> declaredProperties)
        {
            ...
            return declaredProperties.Any(p => p.Name == propertyToLookFor);
        }
        ...
        private void BuildDynamicEpmInfo(ResourceType currentResourceType, ReadOnlyCollection<ResourceProperty> actualTypeDeclaredProperties)
        {
            ...
            if (currentResourceType.HasEntityPropertyMappings)
            {
                ...
                {
                    this.EpmSourceTree.Add(new EntityPropertyMappingInfo(epmAttr, currentResourceType, this, this.epmInfo.EFProvider), actualTypeDeclaredProperties);

                    if (this == currentResourceType)
                    {
                        if (!ResourceType.EpmPropertyExistsInDeclaredProperties(epmAttr, actualTypeDeclaredProperties))
                        {
        ...
        private void AddPropertyInternal(ResourceProperty property)
        {
            ...
            else
            {
                if (property.IsOfKind(ResourcePropertyKind.Key))
                {
                    ...
                    if (typeof(System.Spatial.ISpatial).IsAssignableFrom(property.ResourceType.type))
                    {
        ...
        private void InitializeMetadataAndSchemaVersionForPrimitiveType()
        {
            ...
            {
                // For stream properties, we do not bump up request or response version
                // since we supported MLE in V1.
                this.metadataVersion = VersionUtil.Version3Dot0;
                this.schemaVersion = MetadataEdmSchemaVersion.Version3Dot0;
            }
            else if (typeof(System.Spatial.ISpatial).IsAssignableFrom(this.type))
            {
                // Bump up the metadata version and schema version to 3.0
                // for spatial types.
                this.metadataVersion = VersionUtil.Version3Dot0;
                this.schemaVersion = MetadataEdmSchemaVersion.Version3Dot0;
            }
            ...
            {
                this.metadataVersion = VersionUtil.Version1Dot0;
        ...
        private void InitializeMetadataAndSchemaVersionForCollectionType()
        {
            ...
            // Bump up the metadata version to 3.0 and schema version to 3.0
            // for collection resource type.
            this.metadataVersion = VersionUtil.Version3Dot0;
            this.schemaVersion = MetadataEdmSchemaVersion.Version3Dot0;
        }
        ...
        private void InitializeMetadataAndSchemaVersionForEntityCollectionType()
        {
            ...
            // Entity collections are not versioned, so use V1
            this.metadataVersion = VersionUtil.Version1Dot0;
            this.schemaVersion = MetadataEdmSchemaVersion.Version1Dot0;
        }
        ...
        private void InitializeMetadataAndSchemaVersionForComplexOrEntityType()
        {
            ...
            Version resourceTypeMetadataVersion = VersionUtil.Version1Dot0;
            MetadataEdmSchemaVersion resourceTypeSchemaVersion = MetadataEdmSchemaVersion.Version1Dot0;
        ...
        {
            Version propertyMetadataVersion = VersionUtil.Version1Dot0;
