    ...
    internal sealed class OperationWrapper
    {
        ...
        public void ApplyConfiguration(DataServiceConfiguration configuration, DataServiceProviderWrapper provider)
        {
            ...
            if ((this.Kind == OperationKind.ServiceOperation && (this.serviceOperationRights & ~ServiceOperationRights.OverrideEntitySetRights) != ServiceOperationRights.None) ||
                (this.Kind == OperationKind.Action && this.serviceActionRights != Services.ServiceActionRights.None))
            {
        ...
        internal string GetActionTargetSegmentByResourceType(ResourceType resourceType, string containerName)
        {
            ...
            {
                // If the binding type is an open type, we require the action name to be fully qualified to avoid collision against any open property name.
                // If not, get all properties on this type and any base types and check for name collision.
                if (resourceType.IsOpenType || resourceType.Properties.Any(property => property.Name.Equals(this.Name, StringComparison.Ordinal)))
                {
                    titleSegment = string.Concat(containerName, ".", this.Name);
                }
                else
                {
                    titleSegment = this.Name;
                }
