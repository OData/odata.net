...
namespace System.Data.Services.Providers
{
    ...
    internal static class MetadataProviderUtils
    {
        ...
        {
            { DataServiceProtocolVersion.V1.ToVersion(), new Version(1, 2) },
            { DataServiceProtocolVersion.V2.ToVersion(), new Version(2, 0) },
            { DataServiceProtocolVersion.V3.ToVersion(), new Version(3, 0) },
        };
                ...
                (context, item) =>
                {
                    ...
                    if (propertyName != null && propertyName.IndexOfAny(InvalidCharactersInPropertyNames) >= 0)
                    {
                        ...
                        context.AddError(
                            ...
                            Services.Strings.MetadataProviderUtils_PropertiesMustNotContainReservedChars(propertyName, invalidChars));
                    }
                });
        ...
        private static readonly Version Version1Dot1 = new Version(1, 1);

        /// <summary>Version number for CSDL 1.2.</summary>
        private static readonly Version Version1Dot2 = new Version(1, 2);
        ...
        internal static Version ToVersion(this MetadataEdmSchemaVersion schemaVersion)
        {
            switch (schemaVersion)
            {
                ...
                default:
                    Debug.Fail("Unrecognized schema version.");
                    return Version3Dot0;
            }
        }
        ...
        internal static void ConvertEntityPropertyMappings(MetadataProviderEdmModel model, ResourceType resourceType, EdmEntityType entityType)
        {
            ...
            IEnumerable<EntityPropertyMappingAttribute> epmAttributes = resourceType.AllEpmInfo;
            if (epmAttributes != null)
            {
                ODataEntityPropertyMappingCollection mappings = new ODataEntityPropertyMappingCollection();
                foreach (EntityPropertyMappingAttribute epmAttribute in epmAttributes)
                {
                    mappings.Add(epmAttribute);
                }
        ...
        private static IEdmPrimitiveTypeReference ApplyFacetAnnotations(this IEdmPrimitiveTypeReference primitiveTypeReference, List<KeyValuePair<string, object>> annotations)
        {
            ...
            switch (kind)
            {
                ...
                default:
                    throw new InvalidOperationException(Services.Strings.MetadataProviderUtils_UnsupportedPrimitiveTypeKind(kind.ToString()));
            }
        ...
        private static IEdmPrimitiveTypeReference CreateBinaryTypeReference(IEdmPrimitiveTypeReference primitiveTypeReference, bool nullableFacet, List<KeyValuePair<string, object>> annotations)
        {
            ...
            return new EdmBinaryTypeReference(primitiveTypeReference.PrimitiveDefinition(), nullableFacet, isMaxMaxLength, maxLength, isFixedLength);
        }
        ...
        private static IEdmPrimitiveTypeReference CreateStringTypeReference(IEdmPrimitiveTypeReference primitiveTypeReference, bool nullableFacet, List<KeyValuePair<string, object>> annotations)
        {
            ...
            string collation = GetCollationAnnotation(annotations);
            return new EdmStringTypeReference(primitiveTypeReference.PrimitiveDefinition(), nullableFacet, isMaxMaxLength, maxLength, isFixedLength, isUnicode, collation);
        }
        ...
        private static int? GetMaxLengthAnnotation(List<KeyValuePair<string, object>> annotations, out bool isMaxMaxLength)
        {
            ...
            if (TryFindAndRemoveAnnotation(annotations, XmlConstants.CsdlMaxLengthAttributeName, out maxLengthAnnotationValue))
            {
                ...
                throw new FormatException(Services.Strings.MetadataProviderUtils_ConversionError(XmlConstants.CsdlMaxLengthAttributeName, typeName, typeof(string).FullName));
            }
        ...
        private static string GetCollationAnnotation(List<KeyValuePair<string, object>> annotations)
        {
            ...
            object collationAnnotationValue;
            if (TryFindAndRemoveAnnotation(annotations, XmlConstants.CsdlCollationAttributeName, out collationAnnotationValue))
            {
                return ConvertAnnotationValue<string>(collationAnnotationValue, XmlConstants.CsdlCollationAttributeName);
            }
        ...
        private static T ConvertAnnotationValue<T>(object annotationValue, string facetName)
        {
            ...
            if (!TryConvertAnnotationValue(annotationValue, out convertedValue))
            {
                ...
                throw new FormatException(Services.Strings.MetadataProviderUtils_ConversionError(facetName, typeName, typeof(T).FullName));
            }
