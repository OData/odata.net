...
namespace System.Data.Services.Providers
{
    ...
    internal partial class ObjectContextServiceProvider : BaseServiceProvider, IDataServiceUpdateProvider2, IDataServiceEntityFrameworkProvider
    {
        ...
        /// </summary>
        private static readonly string StoreGeneratedPatternAnnotationName = WebUtil.CreateFullNameForCustomAnnotation(XmlConstants.EdmAnnotationsNamespace, XmlConstants.CsdlStoreGeneratedPattern);
        ...
        public MetadataEdmSchemaVersion EdmSchemaVersion
        {
            get
            {
                double schemaVersion = ((EdmItemCollection)this.ObjectContext.MetadataWorkspace.GetItemCollection(DataSpace.CSpace)).EdmVersion;
                ...
                Debug.Assert(schemaVersion == 3.0, "Invalid EF schema version encountered");
        ...
        public virtual void SetValue(object targetResource, string propertyName, object propertyValue)
        {
            ...
            {
                var resourcePropertyCacheItem = (ObjectContextResourcePropertyCacheItem)this.GetResourcePropertyCacheItem(resourceTypeCacheItem, resourceProperty);

                // Dev11 Bug 138729: The ObjectStateEntry.CurrentValues.SetValue API used below will fail on properties which are being
                // set to null but are marked non-nullable in the model. This is particularly bad for store-generated properties and in
                // particular those that are mapped to Edm.String or Edm.Binary which will be null by default on most clients.
                // Because there is no way to disable this check, we need to skip over the property altogether. We decided we would 
                // only skip the specific case where a non-nullable store-generated property is being set to null.
                if (propertyValue == null && !resourcePropertyCacheItem.IsNullable && resourcePropertyCacheItem.IsStoreGenerated)
                {
                    return;
                }
                ...
                try
                {
                    // Dev11 Bug 41590: The reason for using EF current values api is that in case of POCO, EF will know 
...
#if EF6Provider
            // GetRequiredOriginalValueMembers was obsoleted in EF6, use it here to keep existing ETag computing logic
...
#pragma warning restore 0618
...
#if !EF6Provider
                // TODO: add support for EPM for EF6. Look for attributes and call the public surface area.
                // Same for named streams and other attributes.
                this.GetEpmInfoForResourceType(metadataWorkspace, resourceType);
#endif
            }
        }
            ...
            ProviderMetadataCacheItem metadataCacheItem)
        {
            ...
            // inherit to 'true'.
            foreach (object namedStream in clrType.GetCustomAttributes(typeof(NamedStreamAttribute), baseResourceType == null))
            {
                resourceType.AddProperty(new ResourceProperty(((NamedStreamAttribute)namedStream).Name, ResourcePropertyKind.Stream, PrimitiveResourceTypeMap.TypeMap.GetPrimitive(typeof(System.IO.Stream))));
            }
...
#pragma warning restore 618
        ...
        private void GetEpmInfoForResourceType(MetadataWorkspace workspace, ResourceType resourceType)
        {
            ...
            // For EF all custom annotation in the CSDL gets serialized to the EDMX. So if the CSDL contains EPM mappings on complex types, $metadata would still show
            // those mappings even though we don't initialize the Epm Info here.
            if (resourceType.ResourceTypeKind == ResourceTypeKind.EntityType)
            {
                ...
                foreach (EpmHelper.EpmPropertyInformation propertyInformation in EpmHelper.GetEpmInformationFromType(edmType))
                {
                    ...
                    {
                        throw new InvalidOperationException(Strings.ObjectContext_UnknownPropertyNameInEpmAttributes(propertyInformation.SourcePath, resourceType.Name));
                    }
                ...
                {
                    ResourceProperty resourceProperty = resourceType.TryResolvePropertiesDeclaredOnThisTypeByName(member.Name, exceptKind: ResourcePropertyKind.Stream);
                    Debug.Assert(resourceProperty != null, "resourceProperty must not be null");
                    foreach (EpmHelper.EpmPropertyInformation propertyInformation in EpmHelper.GetEpmInformationFromProperty(member))
                    {
        ...
        private void GetEpmInfoForResourceProperty(
            EpmHelper.EpmPropertyInformation propertyInformation,
            ResourceType resourceType,
            ResourceProperty resourceProperty)
        {
            if (propertyInformation.IsAtom)
            {
                ...
                else
                {
                    ...
                    resourceType.AddEntityPropertyMappingAttributeInternal(epmAttr, true /*isEFProvider*/);
                }
            }
            else
            {
        ...
        private ResourceProperty GetResourcePropertyFromEpmPath(ResourceType baseResourceType, String sourcePath)
        {
            ...
            if (baseResourceType.TryResolvePropertiesDeclaredOnThisTypeByName(propertyPath[0], exceptKind: ResourcePropertyKind.Stream) == null)
            {
            ...
            else
            {
                ...
                {
                    resourceProperty = baseResourceType.TryResolvePropertiesDeclaredOnThisTypeByName(pathSegment, exceptKind: ResourcePropertyKind.Stream);
        ...
        private class ObjectContextResourcePropertyCacheItem : ResourcePropertyCacheItem
        {
            ...
            private readonly bool isStoreGenerated;
                ...
                : base(propertyInfo)
            {
                if (providerMember.EdmTypeKind == BuiltInTypeKind.PrimitiveType)
                {
                    ...
                    this.isStoreGenerated = PropertyIsStoreGenerated(providerMember.MetadataProperties);
                }
                else
                {
                    this.isNullable = false;
                    this.isStoreGenerated = false;
                }
            }
            ...
            {
                get { return this.isStoreGenerated; }
            }
            ...
            private static bool PropertyIsStoreGenerated(IEnumerable<MetadataProperty> metadataProperties)
            {
                foreach (MetadataProperty metadataProperty in metadataProperties)
                {
                    if (metadataProperty.PropertyKind != PropertyKind.System && metadataProperty.Name == StoreGeneratedPatternAnnotationName)
                    {
                        if ((string)metadataProperty.Value == XmlConstants.CsdlStoreGeneratedPatternIdentity ||
                            (string)metadataProperty.Value == XmlConstants.CsdlStoreGeneratedPatternComputed)
                        {
                            return true;
                        }
