namespace __GeneratedOdataV2.Trancsribers.Rules
{
    public sealed class _divExprTranscriber : GeneratorV3.ITranscriber<__GeneratedOdataV2.CstNodes.Rules._divExpr>
    {
        private _divExprTranscriber()
        {
        }
        
        public static _divExprTranscriber Instance { get; } = new _divExprTranscriber();
        
        public void Transcribe(__GeneratedOdataV2.CstNodes.Rules._divExpr value, System.Text.StringBuilder builder)
        {
            __GeneratedOdataV2.Trancsribers.Rules._RWSTranscriber.Instance.Transcribe(value._RWS_1, builder);
__GeneratedOdataV2.Trancsribers.Inners._ʺx64x69x76ʺTranscriber.Instance.Transcribe(value._ʺx64x69x76ʺ_1, builder);
__GeneratedOdataV2.Trancsribers.Rules._RWSTranscriber.Instance.Transcribe(value._RWS_2, builder);
__GeneratedOdataV2.Trancsribers.Rules._commonExprTranscriber.Instance.Transcribe(value._commonExpr_1, builder);

        }
    }
    
}
