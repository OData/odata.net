namespace __GeneratedOdata.CstNodes.Inners
{
    public sealed class _COMMA_functionParameter
    {
        public _COMMA_functionParameter(__GeneratedOdata.CstNodes.Rules._COMMA _COMMA_1, __GeneratedOdata.CstNodes.Rules._functionParameter _functionParameter_1)
        {
            this._COMMA_1 = _COMMA_1;
            this._functionParameter_1 = _functionParameter_1;
        }
        
        public __GeneratedOdata.CstNodes.Rules._COMMA _COMMA_1 { get; }
        public __GeneratedOdata.CstNodes.Rules._functionParameter _functionParameter_1 { get; }
    }
    
}
