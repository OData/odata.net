...
namespace V2.Fx
{
    public static class BetterReadOnlySpan
    {
        ...
        public static BetterReadOnlySpan<T> FromInstance<T>(scoped ref T value) where T : allows ref struct //// TODO is `scoped` ok here? //// TODO is there a way to do this without `in`? //// TODO removing either `scoped` or `in` might cause a test to fail, so you should have a memory integrity issue in mind that removing them fixes
        {
